...
/-- Exact compiled finite-profile-cover target now exposed by the σ-dependent route:
choose a finite active family of realized coarse signatures and a finite basis inside
each realized bucket slice, with uniform `n^120` cardinality, whose union still spans
the logscale blocked-SPDP subspace of the compiled Cook-Levin polynomial. -/
def latent_compiled_realized_signature_cover_uniform120_logscale (M : DTM) (n : ℕ)
    (_hn : n ≥ max 4 M.numStates)
    (_hn804 : n ≥ 2 ^ 804) : Prop :=
  ∃ (Iσ : Finset (latentProfileSignature M n))
    (Gσ : latentProfileSignature M n → Finset (MvPolynomial (Fin (latentNumVars M n)) ℚ)),
      mlBlockedSpdpSubspace (latentPartition M n) (Nat.log 2 n) (Nat.log 2 n)
        (latentCompiledPoly M n)
        ≤ Submodule.span ℚ (↑(Iσ.biUnion Gσ)
            : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ)) ∧
      ∀ σ ∈ Iσ, (Gσ σ).card ≤ n ^ 120

/-- A finite compiled generator witness together with the per-signature within-profile
finrank theorem yields the exact realized-signature finite-profile cover:
extract the coarse signature realized by each chosen generator, take a basis of the
finite span of that realized bucket, and union those bounded bases. -/
theorem latent_compiled_realized_signature_cover_uniform120_from_witness_construction_data
    (M : DTM) (n : ℕ)
    (hn : n ≥ max 4 M.numStates)
    (hn804 : n ≥ 2 ^ 804)
  ...
  let bucketOf : latentProfileSignature M n →
      Finset (MvPolynomial (Fin (latentNumVars M n)) ℚ) :=
    fun σ => hData.G.filter (fun g => realizedSig g = σ)
  let basisOf : latentProfileSignature M n →
      Finset (MvPolynomial (Fin (latentNumVars M n)) ℚ) := fun σ =>
    let S : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ) := ↑(bucketOf σ)
    letI : Module.Finite ℚ (Submodule.span ℚ S) :=
      Module.Finite.span_of_finite ℚ (Finset.finite_toSet (bucketOf σ))
    Classical.choose (Submodule.exists_finset_span_eq_linearIndepOn ℚ S)
  have hbasis_span :
      ∀ σ,
        Submodule.span ℚ
            (↑(basisOf σ) : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ)) =
          Submodule.span ℚ
            (↑(bucketOf σ) : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ)) := by
    intro σ
    dsimp [basisOf]
    let S : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ) := ↑(bucketOf σ)
    letI : Module.Finite ℚ (Submodule.span ℚ S) :=
      Module.Finite.span_of_finite ℚ (Finset.finite_toSet (bucketOf σ))
    rcases (Classical.choose_spec (Submodule.exists_finset_span_eq_linearIndepOn ℚ S)) with
      ⟨_hsub, _hcard, hspan, _hlin⟩
    exact hspan
  have hbasis_card :
      ∀ σ,
        (basisOf σ).card =
    ...
    letI : Module.Finite ℚ (Submodule.span ℚ S) :=
      Module.Finite.span_of_finite ℚ (Finset.finite_toSet (bucketOf σ))
    rcases (Classical.choose_spec (Submodule.exists_finset_span_eq_linearIndepOn ℚ S)) with
      ⟨_hsub, hcard, _hspan, _hlin⟩
    exact hcard
  have hbucket_span_le_slice :
      ∀ σ,
        Submodule.span ℚ
            (↑(bucketOf σ) : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ)) ≤
          latent_fixedProfileSlice M n σ := by
    intro σ
    apply Submodule.span_mono
    intro g hg
    have hgG : g ∈ hData.G := (Finset.mem_filter.mp hg).1
    have hEq : realizedSig g = σ := (Finset.mem_filter.mp hg).2
    have hmem : g ∈ latent_profile_bucket_generators M n (realizedSig g) :=
      hrealized_mem_bucket g hgG
    simpa [hEq] using hmem
  have hbasis_card_bound : ∀ σ ∈ Iσ, (basisOf σ).card ≤ n ^ 120 := by
    intro σ hσ
    letI : Module.Finite ℚ (latent_fixedProfileSlice M n σ) := hBucketFinite σ
    calc
      (basisOf σ).card =
          Module.finrank ℚ
            (Submodule.span ℚ
              (↑(bucketOf σ) : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ))) :=
        hbasis_card σ
      _ ≤ Module.finrank ℚ (latent_fixedProfileSlice M n σ) :=
        Submodule.finrank_mono (hbucket_span_le_slice σ)
      _ ≤ n ^ 120 := hBucket σ
  have hG_span_le_union :
      Submodule.span ℚ (↑hData.G : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ)) ≤
        Submodule.span ℚ
          (↑(Iσ.biUnion basisOf) : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ)) := by
    apply Submodule.span_le.mpr
    intro g hg
    have hg_bucket : g ∈ bucketOf (realizedSig g) := Finset.mem_filter.mpr ⟨hg, rfl⟩
    have hg_span_bucket :
        g ∈ Submodule.span ℚ
          (↑(bucketOf (realizedSig g)) : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ)) :=
      Submodule.subset_span hg_bucket
    have hg_span_basis :
        g ∈ Submodule.span ℚ
          (↑(basisOf (realizedSig g)) : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ)) := by
      simpa [hbasis_span (realizedSig g)] using hg_span_bucket
    have hbasis_subset_union :
        (↑(basisOf (realizedSig g)) : Set (MvPolynomial (Fin (latentNumVars M n)) ℚ)) ⊆
          ↑(Iσ.biUnion basisOf) := by
      intro q hq
      exact Finset.mem_biUnion.mpr ⟨realizedSig g, Finset.mem_image.mpr ⟨g, hg, rfl⟩, hq⟩
    exact (Submodule.span_mono hbasis_subset_union) hg_span_basis
...
/-- The latent hit-block set of a copy-slot witness list is exactly the underlying base-index
set. This is the signature-level bridge from cleaned copy witnesses back to the actual base
blocks they touch. -/
theorem latent_hitBlocks_of_copySlot_map
    (M : DTM) (n : ℕ)
    (ks : List (Fin (latentBaseVars M n))) :
    latent_hitBlocks_of_list M n (ks.map (copySlot M n)) = ks.toFinset := by
  unfold latent_hitBlocks_of_list
  have hmap : ks.map (fun i => (latentPartition M n).assign (copySlot M n i)) = ks := by
    apply List.ext_getElem <;> simp [latentPartition_assign_copySlot]
  simpa [List.map_map] using congrArg List.toFinset hmap

/-- The latent hit-block set of a pure-`conSlot` witness list is exactly the underlying
base-index set. -/
theorem latent_hitBlocks_of_conSlot_map
    (M : DTM) (n : ℕ)
    (ks : List (Fin (latentBaseVars M n))) :
    latent_hitBlocks_of_list M n (ks.map (conSlot M n)) = ks.toFinset := by
  unfold latent_hitBlocks_of_list
  have hmap : ks.map (fun i => (latentPartition M n).assign (conSlot M n i)) = ks := by
    apply List.ext_getElem <;> simp [latentPartition_assign_conSlot]
  simpa [List.map_map] using congrArg List.toFinset hmap

/-- Shared coarse signature forces a pure-`conSlot` witness list and a cleaned copy-slot witness
list to touch exactly the same base blocks. This is the structural alignment needed before any
coefficient-level comparison on `copyConSheet`. -/
theorem latent_same_signature_con_vs_copy_toFinset
    (M : DTM) (n : ℕ)
    (σ : latentProfileSignature M n)
    (kscon kscopy : List (Fin (latentBaseVars M n)))
    (mcon mcopy : MvPolynomial (Fin (latentNumVars M n)) ℚ)
    (hLenCon : kscon.length = Nat.log 2 n)
    (hDegCon : mcon.totalDegree ≤ Nat.log 2 n)
    (hLenCopy : kscopy.length = Nat.log 2 n)
    (hDegCopy : mcopy.totalDegree ≤ Nat.log 2 n)
    (hSigCon : latent_profile_signature_of_generator_data M n (kscon.map (conSlot M n)) mcon
      (by simpa [List.length_map] using hLenCon) hDegCon = σ)
    (hSigCopy : latent_profile_signature_of_generator_data M n (kscopy.map (copySlot M n)) mcopy
      (by simpa [List.length_map] using hLenCopy) hDegCopy = σ) :
    kscon.toFinset = kscopy.toFinset := by
  have hconBlocks :
      (latent_profile_signature_of_generator_data M n (kscon.map (conSlot M n)) mcon
        (by simpa [List.length_map] using hLenCon) hDegCon).hitBlocks = kscon.toFinset := by
    calc
      (latent_profile_signature_of_generator_data M n (kscon.map (conSlot M n)) mcon
        (by simpa [List.length_map] using hLenCon) hDegCon).hitBlocks =
          latent_hitBlocks_of_list M n (kscon.map (conSlot M n)) := by
            exact latent_profile_signature_of_generator_data_hitBlocks M n
              (kscon.map (conSlot M n)) mcon
              (by simpa [List.length_map] using hLenCon) hDegCon
      _ = kscon.toFinset := latent_hitBlocks_of_conSlot_map M n kscon
  have hcopyBlocks :
      (latent_profile_signature_of_generator_data M n (kscopy.map (copySlot M n)) mcopy
        (by simpa [List.length_map] using hLenCopy) hDegCopy).hitBlocks = kscopy.toFinset := by
    calc
      (latent_profile_signature_of_generator_data M n (kscopy.map (copySlot M n)) mcopy
        (by simpa [List.length_map] using hLenCopy) hDegCopy).hitBlocks =
          latent_hitBlocks_of_list M n (kscopy.map (copySlot M n)) := by
            exact latent_profile_signature_of_generator_data_hitBlocks M n
              (kscopy.map (copySlot M n)) mcopy
              (by simpa [List.length_map] using hLenCopy) hDegCopy
      _ = kscopy.toFinset := latent_hitBlocks_of_copySlot_map M n kscopy
  calc
    kscon.toFinset =
        (latent_profile_signature_of_generator_data M n (kscon.map (conSlot M n)) mcon
          (by simpa [List.length_map] using hLenCon) hDegCon).hitBlocks := hconBlocks.symm
    _ = σ.hitBlocks := by simpa [hSigCon]
    _ =
        (latent_profile_signature_of_generator_data M n (kscopy.map (copySlot M n)) mcopy
          (by simpa [List.length_map] using hLenCopy) hDegCopy).hitBlocks := by
        simpa [hSigCopy]
    _ = kscopy.toFinset := hcopyBlocks

/-- Concrete comparison data package for the remaining pure-`conSlot` versus cleaned-copy
same-`q` frontier. Starting from a raw pure-`conSlot` presentation and an explicit clean
copy-slot presentation computing the same coarse signature, this theorem normalizes the pure
side to a base-index list, aligns the hit-block sets, and exposes both `copyConSheet` closed
forms in the same residual-times-varying format. -/
theorem latent_pure_conSlot_vs_clean_copy_same_q_aligned_copyCon_forms
    (M : DTM) (n : ℕ)
    (σ : latentProfileSignature M n)
    (q : MvPolynomial (Fin (latentNumVars M n)) ℚ)
    (Scon : List (Fin (latentNumVars M n)))
    ...
    ∃ (kscon : List (Fin (latentBaseVars M n)))
      (residualCon varyingCon residualCopy varyingCopy :
        MvPolynomial (Fin (latentNumVars M n)) ℚ),
      kscon.Nodup ∧
  ...
  refine ⟨kscon,
    ...
    C ((-1 : ℚ)^kscopy.length) * (∏ i ∈ (Finset.univ \ kscopy.toFinset), copyConGadget M n i),
...
/-- The raw pure-con versus clean-copy same-`q` frontier now factors through a three-piece bridge
stack on the aligned copyCon shell:
1. the two aligned `copyConSheet` realizations agree,
...
/-- Canonical P-data package from the compiled-tableau frontier object. -/
theorem theorem216_profile_data_logscale_from_compiled_tableau_proved (M : DTM) (n : ℕ)
    (hn : n ≥ max 4 M.numStates) (hn804 : n ≥ 2 ^ 804)
    (hCompiled : latent_compiled_tableau_bound_logscale M n hn hn804) :
    theorem216_profile_data_logscale M n hn hn804 := by
  exact theorem216_profile_data_logscale_from_core_proved M n hn hn804
    ((latent_profile_assembly_logscale_iff_compiled_tableau_bound M n hn hn804).2 hCompiled)
