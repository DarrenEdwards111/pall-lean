...
new downstream handoff genuinely belongs to the strengthened selector-signature package, not to the
older witness-construction package. -/
...
/-- The new honest downstream target is no longer a theorem from the older witness-construction
package. It is the stronger selector-signature construction-data handoff established above. -/
def local_selector_signature_construction_data_yields_shared_selector_signature_target_candidate
    (M : DTM) (n : ℕ)
    (hn : n ≥ max 4 M.numStates)
    (hn804 : n ≥ 2 ^ 804) : Prop :=
  False
...
/-- Step-3 API hookup: same rank transfer as `global_latent_rank160...`, but
expressed through the `LatentFullBridge` interface predicates/lemmas. -/
theorem global_latent_rank160_of_full_compiled_rank160_via_bridge_api
    (hLeWitness : ∀ (M : DTM) (n : ℕ),
      (hn : n ≥ max 4 M.numStates) → (hn804 : n ≥ 2 ^ 804) →
      npNumVars n ≤ numVars M n (Nat.log 2 n))
    ...
    (hDom : ∀ (M : DTM) (n : ℕ),
      (hn : n ≥ max 4 M.numStates) → (hn804 : n ≥ 2 ^ 804) →
      bridgeRankDomination M n (hLeWitness M n hn hn804) (hBridgeObj M n))
    (hFull : ∀ (M : DTM) (n : ℕ),
      (hn : n ≥ max 4 M.numStates) → (hn804 : n ≥ 2 ^ 804) →
      mlBlockedSpdpRank (compiledPartition M n) (Nat.log 2 n) (Nat.log 2 n)
        (fullCompiledPoly ℚ M n (hLeWitness M n hn hn804)) ≤ n ^ 160) :
    ∀ (M : DTM) (n : ℕ),
      ...
      mlBlockedSpdpRank (latentPartition M n) (Nat.log 2 n) (Nat.log 2 n)
        (latentCompiledPoly M n) ≤ n ^ 160 := by
  intro M n hn hn804
  exact latent_rank_bound_of_full_rank_bound_of_bridge M n
...
/-- Step-3 closure wrapper through the `LatentFullBridge` API. -/
theorem no_PeqNP_of_full_compiled_rank160_via_bridge_api
    (hLeWitness : ∀ (M : DTM) (n : ℕ),
      (hn : n ≥ max 4 M.numStates) → (hn804 : n ≥ 2 ^ 804) →
      npNumVars n ≤ numVars M n (Nat.log 2 n))
    (hBridgeObj : ∀ (M : DTM) (n : ℕ), FullToLatentBridge M n)
    ...
    (hFull : ∀ (M : DTM) (n : ℕ),
      ...
      mlBlockedSpdpRank (compiledPartition M n) (Nat.log 2 n) (Nat.log 2 n)
        (fullCompiledPoly ℚ M n (hLeWitness M n hn hn804)) ≤ n ^ 160) :
    PeqNP → False :=
  no_PeqNP_of_full_compiled_rank160_under_embedding hLeWitness
    (fun M n hn hn804 =>
      latent_rank_le_full_rank_of_bridge M n (hLeWitness M n hn hn804)
        (hBridgeObj M n) (hDom M n hn hn804))
    hFull

/-- Step-2 packaged closure: use the existential bridge API constructor from
`LatentFullBridge` and immediately feed it to the bridge-api closure theorem. -/
theorem no_PeqNP_of_full_compiled_rank160_with_packaged_bridge
    (hLeVar : ∀ (M : DTM) (n : ℕ),
...
(3) full-side rank160 theorem.
This composes all bridge constructors and closes contradiction in one theorem. -/
theorem no_PeqNP_of_full_compiled_rank160_from_core_bridge_obligations
    (hLeVar : ∀ (M : DTM) (n : ℕ),
      numVars M n (Nat.log 2 n) ≤ latentNumVars M n)
    (hLeWitness : ∀ (M : DTM) (n : ℕ),
      (hn : n ≥ max 4 M.numStates) → (hn804 : n ≥ 2 ^ 804) →
...
/-- Same semantic closure as `no_PeqNP_of_semantic_transport_obligations`, but
using the explicit pointwise block-map condition
`compiled.assign i = latent.assign (toLatent i)` instead of global
`bridgePartitionCompatible`.

This is the strengthened/explicit hypothesis path for Step-(2). -/
theorem no_PeqNP_of_semantic_transport_obligations_via_assignToLatent
    (hNP : ∀ (M : DTM) (n : ℕ),
      (hn : n ≥ max 4 M.numStates) → (hn804 : n ≥ 2 ^ 804) →
      npNumVars n ≤ (n ^ M.timeBound + 1) ^ 2)
    (hAssignToLatent : ∀ (M : DTM) (n : ℕ)
      ...
      (compiledPartition M n).assign i =
        (latentPartition M n).assign
          ((fullToLatentBridgeOfLe M n (global_numVars_le_latentNumVars M n)).toLatent i))
    (hVerVanish : ∀ (M : DTM) (n : ℕ)
      (hn : n ≥ max 4 M.numStates) (hn804 : n ≥ 2 ^ 804),
      MvPolynomial.rename
          (Function.comp
  ...
  have hMapDom : ∀ (M : DTM) (n : ℕ)
      ...
      bridgeRankDominationMapped M n (hLeW M n hn hn804)
        (fullToLatentBridgeOfLe M n (global_numVars_le_latentNumVars M n)) :=
    globalMapDom_of_globalAssignToLatent
      global_numVars_le_latentNumVars hLeW hAssignToLatent
  have hLatentDecomp : ∀ (M : DTM) (n : ℕ)
      (hn : n ≥ max 4 M.numStates) (hn804 : n ≥ 2 ^ 804),
      latentCompiledPoly M n
        = MvPolynomial.rename
...
/-- Global closure theorem:
if the Item-3 + uniform-120 profile package is available uniformly at
contradiction scale, then `PeqNP` is impossible via the span160 route. -/
theorem no_PeqNP_of_uniform_item3_uniform120
    (hItem3120 : ∀ (h : PeqNP) (n : ℕ),
      (hn : n ≥ max (max 32 (max 4 h.sat_decider.numStates)) (2 ^ 804)) →
      latent_profile_block_cover_item3_uniform120_logscale h.sat_decider n
        (hnM_of_hn h n hn) (hn804_of_hn h n hn)) :
    PeqNP → False := by
  intro h
  let n : ℕ := max (max 32 (max 4 h.sat_decider.numStates)) (2 ^ 804)
  ...
  exact P_neq_NP_latent_from_p_span160 h n hn
    (latent_p_witness_span160_logscale_from_item3_uniform120 h.sat_decider n
      (hnM_of_hn h n hn) (hn804_of_hn h n hn) (hItem3120 h n hn))
...
theorem no_PeqNP_of_uniform_concrete_locality_profile_structure120_via_span160
    (hLoc120 : ∀ (h : PeqNP) (n : ℕ),
      ...
      concrete_locality_profile_structure120_logscale h.sat_decider n
        (hnM_of_hn h n hn) (hn804_of_hn h n hn)) :
    PeqNP → False := by
  exact no_PeqNP_of_uniform_item3_uniform120 hLoc120
