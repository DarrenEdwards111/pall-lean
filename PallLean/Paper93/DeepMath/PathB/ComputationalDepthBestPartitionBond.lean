...
genuine statement, `∃ f, ∀ balanced S, 2^r ≤ finrank(span(range (residualOf S f)))`, **is** now proved:
`GlobalBestPartitionBond.exists_global_best_partition_bond` (via the global quadratic form `QF A`, the residual
factorization `residualOf S (QF A) α = D·c(α)·χ_{y(α)}`, and a probabilistic existence over the **symmetrized**
block `(A+Aᵀ)[S][Sᶜ]`).
...
global `∃ f, ∀ S` statement is `GlobalBestPartitionBond.exists_global_best_partition_bond`. -/
