...
/-- Route-B closeout from the type-slot/factor exact-budget atom-trace
coded-basis seam.  This lowers the fibre partition to an explicit per-type
equivalence between atom-trace slots and concrete Cook--Levin factors, carrying
the selected shift/`mlProj` closure as the remaining algebraic obligation. -/
theorem P_ne_NP_canonical_routeB_touchedAtomTraceExactTypeSlotFactorCodedBasis_conditional
...
does not pretend to be elementary.  Its custom trust surface now follows the
...
single `GlobalGodMoveGauge.exists_amplituhedron_gauge` spec. The same-sheet
polynomial is the projected Cook-Levin polynomial, the P-side/NP-side bounds
come from the chosen gauge, and extraction rank-monotonicity is supplied by the
identity-extraction constructor. -/
...
All non-elementary content is kept in the named amplituhedron-gauge existence
