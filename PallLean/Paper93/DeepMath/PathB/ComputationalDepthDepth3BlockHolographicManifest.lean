...
-- Brick 39: branching holography, step 2b — the deepest input realises the height
...
-- Brick 41: branching holography, step 3b — the sequential decoder
...
-- Brick 43: branching holography, step 4a — the encoding function is injective
#check @Depth3.descent_encode_injective          -- σ ↦ (labels, final) is injective
...
-- Brick 57: branching holography, step 4o — the boundary fixes ≥ s variables on the bad event
...
-- AC⁰ reduction brick 2 (step 5b): the dual decision-tree → CNF conversion
#check @Depth3.cnfValue          -- CNF semantics (every clause has a true literal)
#check @Depth3.dtreeToCNF        -- rejecting-path CNF of a decision tree
...
-- AC⁰ reduction brick 32 (step 6f): the SCALED CONDITIONAL switching bound (makes the budget uniform)
#check @Depth3.descent_switching_prob_extends  -- boundary sum restricted to extBox τ via descentSat extends
#check @Depth3.descent_switching_le_extends    -- ∑_{Bad⊆extBox τ} pweight ≤ cap · ((1-p)/2)^(n-stars τ)
-- THE FIX for iteration. Brick 23/25 used the UNCONDITIONAL cap, so the budget #gates·cap < ((1-p)/2)^(n-
-- stars τ) shrinks exponentially each round (unsatisfiable for round ≥ 2). Here the cap is SCALED by the
...
-- AC⁰ reduction brick 34 (step 6h): the UNIFORM CONDITIONAL SWITCHING PRIMITIVE (complete iterable step)
#check @Depth3.exists_shallow_survivor_extends_unif
-- Budget #gates·cap·t^k + (t·p+(1-p))^(stars τ) < t^k ⟹ ∃ ρ extending τ, every gate shallow, k < stars ρ.
-- Combines the scaled deep bound (brick 32) and the conditional star tail (brick 24), both over the
-- conditioning mass which CANCELS (no ((1-p)/2)^(…) factor remains). The complete iterable switching step:
-- shallow + survivors + extends τ, under a mass-free budget the d-round loop reuses with ONE parameter set.
...
-- AC⁰ reduction brick 86 (step 46): the MERGE PASS (depth reduction after the leaf-switch)
#check @Depth3.mergePass_EquivOn
-- leafCollapse (85) switches bottom gates but keeps the tree shape; the depth drop comes from MERGING the
-- same-type siblings the switch creates (a bottom gAnd-of-cnf → cnf, gOr-of-dnf → dnf, one level shorter).
-- allCnf/allDnf detect a uniform child list + extract clause-lists (allCnf_some : allCnf gs = some css → gs =
-- css.map cnf). mutual mergePass/mergePassList: gAnd gs → (allCnf gs).elim recurse (cnf css.flatten); gOr dual.
-- mergePass_EquivOn (ρ) : ∀C, EquivOn ρ C (mergePass C) — UNCONDITIONAL (merges are eval-preserving): some-case
-- via merge_gAnd_cnf_EquivOn/merge_gOr_dnf_EquivOn (81) after allCnf_some, none-case via congruence. Mutual
...
-- AC⁰ reduction brick 96 (step 56): THE MERGE PRESERVES THE BOTTOM-GATE WIDTH (width-aware merge)
#check @Depth3.BottomWidth
#check @Depth3.mergePass_BottomWidth
-- The merge half of the width-aware reduction. KEY SIMPLIFICATION: only WIDTH needs threading — the clause-
-- count is read off the final DNF (hsmall stays a hypothesis, and p=1/(8w·count) makes the rate hr1 hold at
-- ANY count). BottomWidth w C := ∀ cs ∈ bottomGates C, ∀T∈cs, T.lits.length ≤ w (the width projection of
-- BottomBounded, via BottomBounded_BottomWidth). mergePass_BottomWidth (mutual with mergePassList_BottomWidth):
-- mergePass preserves BottomWidth — the some-case flattens gAnd-of-cnf/gOr-of-dnf into one gate whose terms are
-- exactly the children's (T∈css.flatten → ∃c∈css, T∈c, width≤w via BottomWidth_child_gAnd projection); the
-- none-case recurses through children. So a merge keeps width=max≤w. With leafCollapse_*_survivor (95) setting
-- width≤s, collapseRound (switch+merge) keeps the tower BottomWidth s. Next: collapseRound_BottomWidth (compose
