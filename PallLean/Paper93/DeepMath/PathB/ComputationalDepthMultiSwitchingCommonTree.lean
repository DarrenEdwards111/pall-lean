...
theorem queryVars_length_le_depth {n : ℕ} {α : Type}
    (t : CommonTree n α) (x : Fin n → Bool) :
    (queryVars t x).length ≤ depth t := by
  induction t with
  | leaf a => simp [queryVars, depth]
  | query i lo hi ihlo ihhi =>
      by_cases hxi : x i
      · simp only [queryVars, hxi, if_true, List.length_cons, depth]
        omega
      · simp only [queryVars, hxi, Bool.false_eq_true, if_false,
          List.length_cons, depth]
        omega

/-- Changing a coordinate absent from the followed query path does not change the reached leaf. -/
theorem run_update_of_not_mem_queryVars {n : ℕ} {α : Type}
    (t : CommonTree n α) (x : Fin n → Bool) (i : Fin n)
    (hi : i ∉ queryVars t x) :
    run t (Function.update x i (!x i)) = run t x := by
  induction t with
  | leaf a => rfl
  | query j lo hiTree ihlo ihhi =>
      by_cases hxj : x j
      · simp only [queryVars, hxj, if_true, List.mem_cons, not_or] at hi
        have hji : j ≠ i := Ne.symm hi.1
        have hupdate : Function.update x i (!x i) j = x j :=
          Function.update_of_ne hji _ _
        simpa only [run, hupdate, hxj, if_true] using ihhi hi.2
      · simp only [queryVars, hxj, Bool.false_eq_true, if_false,
          List.mem_cons, not_or] at hi
        have hji : j ≠ i := Ne.symm hi.1
        have hupdate : Function.update x i (!x i) j = x j :=
          Function.update_of_ne hji _ _
        simpa only [run, hupdate, hxj, Bool.false_eq_true, if_false] using ihlo hi.2
...
#print axioms PallLean.Paper93.DeepMath.PathB.MultiSwitching.CommonTree.queryVars_length_le_depth
#print axioms PallLean.Paper93.DeepMath.PathB.MultiSwitching.CommonTree.run_update_of_not_mem_queryVars
