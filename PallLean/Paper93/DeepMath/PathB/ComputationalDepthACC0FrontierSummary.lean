...
/-- **Bridge (explicit exponential): `MOD_q` size reaches `p^t` by a polynomial arity `modq_size_blowup` (PROVED).**  The
explicit exponential size lower bound: for any uniform `AC⁰[p]` family computing `MOD_q` at depth `d` and any `t`, within
arities `N ≤ 16((p−1)t)^d)²+1+q` (polynomial in `t`) some circuit has `p^t < 4q·(subcircuits (toBoolSyntax (D N))).length` —
i.e. `size(n) ≥ p^{Ω(n^{1/2d})}`. -/
theorem nw_modq_size_blowup_frontier (p q : ℕ) [Fact p.Prime] [Fact q.Prime] (hpq : ¬ q ∣ p)
    {d : ℕ} (D : (N : ℕ) → ACC0CircuitModel.ACC0Circuit N)
    (hDind : ∀ N, ∀ y : Fin N → Bool, ACC0CircuitModel.eval (D N) y
      = decide ((Finset.univ.filter (fun i => y i = true)).card % q = 0))
    (hDmod : ∀ N, ACC0CircuitReprP.ModpOnly p (D N))
...
/-- **Bridge (AC⁰ ⊊ AC⁰[2]): the constant-depth hierarchy is strict `ac0_strict_subset_acc02` (PROVED).**  `PARITY` is
computed by a single `MOD_2` gate (`parity_mem_acc02`, depth 1) so lies in `AC⁰[2]`, but `PARITY ∉ AC⁰` (`parity_not_ac0`).
Hence `MOD` gates strictly increase the power of constant-depth circuits: `AC⁰ ⊊ AC⁰[2]`. -/
theorem nw_ac0_strict_subset_acc02_frontier {n d : ℕ} (hd : 2 * 2 ^ d < n) :
...
/-- **Bridge (AC⁰ ⊊ AC⁰[2], real model): strict separation in unbounded fan-in `ac0_strict_subset_acc02_unbounded`
(PROVED).**  `PARITY` is a depth-`1` `AC⁰[2]` circuit (one unbounded-fan-in `MOD_2` gate = `toBoolSyntax (mod 2 univ 1)`),
but every depth-`d` `AC⁰` circuit computing it has `3^t < 4·#{subcircuits}` (super-poly).  So `AC⁰ ⊊ AC⁰[2]` in the real
model. -/
theorem nw_ac0_strict_subset_acc02_unbounded_frontier {m d t : ℕ} (ht1 : 1 ≤ t)
    (hm : 8 * ((2 * t) ^ d) ^ 2 ≤ m) :
