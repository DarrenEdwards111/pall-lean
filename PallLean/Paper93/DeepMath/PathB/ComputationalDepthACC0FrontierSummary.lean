...
```
 ...
 Route A: rank-shrink ⇒ holonomy lower bound   PROVED   nframe_rank_route_frontier          (…ACC0RankRouteFrontier)
   ...
   ── DYNAMIC OBSERVER SELECTION: dynamicObserver_refines_modp_to_padic_frontier — formalises N-Frame's principle. ─────
   ──   Sufficient/Finer observer abstraction. PROVED: residue observer (mod p) NOT sufficient for MOD_{p^e} (forced ──
   ...
   ── hstep MACHINE (explicit overhead): hstep_realized_with_overhead_frontier — given the 4 per-phase realizations ────
   ──   (decode c0→c1, lookup c1→c2, rewrite c2→c3, re-encode c3→c4, each a U-transition run within its bound), U ──────
   ...
   ── CHERNOFF BOUND: nw_binomial_tail_le_frontier — the combinatorial Chernoff inequality (PROVED, NO measure theory): ─
   ──   ∑_{j≤m} C(k,j)·a^j·b^{k−j} ≤ 2^k·a^m·b^{k−m} (b≤a, m≤k). term_mono (top tail term dominates) + ∑C(k,j)≤2^k ────
...
/-- **CRT-generalised composite `MOD` gate polynomial (proved).**  The general prime-modulus gate `modPGate p =
1 − X^{p−1}` (degree `p−1` over `F_p`) computes `MOD_p` (`eval (s) = 1 ↔ p ∣ s`), and for distinct primes `p ≠ q` both
gates fire iff `p·q ∣ s` (coprime CRT) — `MOD₆` generalised to arbitrary squarefree composite modulus. -/
theorem modPQ_gate_decides_frontier (p q : ℕ) [Fact p.Prime] [Fact q.Prime] (hpq : p ≠ q) (s : ℕ) :
    (MvPolynomial.eval (fun _ => (s : ZMod p)) (ACC0CRTGatePolys.modPGate p) = 1
        ∧ MvPolynomial.eval (fun _ => (s : ZMod q)) (ACC0CRTGatePolys.modPGate q) = 1)
      ↔ (p * q) ∣ s :=
...
/-- **Prime-power `MOD`: CRT divisibility generalises (proved), the low-degree field gate does not (obstruction).**
`(p^a · q^b) ∣ s ↔ (p^a ∣ s ∧ q^b ∣ s)` (coprime prime powers) and the ring residue observer `(s : ZMod (p^e)) = 0 ↔
p^e ∣ s` both generalise; but `ZMod (p^e)` is not a field, so there is no Fermat-style low-degree gate polynomial — the
genuine `ACC⁰[composite]` obstruction (squarefree worked by avoiding prime powers). -/
theorem primePower_dvd_frontier (p q a b : ℕ) (hp : p.Prime) (hq : q.Prime) (hpq : p ≠ q) (s : ℕ) :
...
/-- **Dynamic observer selection: the boundary refines `MOD_p`'s observer to the `p`-adic observer for `MOD_{p^e}`
(proved).**  The residue observer `s ↦ s mod p` is *not* sufficient for `MOD_{p^e}` (`e≥2`), the `p`-adic carry observer
`s ↦ (⌊s/p^i⌋ mod p)_{i<e}` *is* sufficient, and it refines the residue observer — so the boundary is forced to refine
field-residue → carry-aware `p`-adic.  (Carry observer → threshold observer next; closure at a quasipoly BT observer is
the socketed wall, `dynamic_boundary_to_acc0_sat_speedup`.) -/
theorem dynamicObserver_refines_modp_to_padic_frontier (p e : ℕ) (hp : p.Prime) (he : 2 ≤ e) :
...
/-- **Physical `hstep` realization with explicit overhead `B` (proved).**  Given the four per-phase realizations
(decode `c0→c1`, lookup `c1→c2`, rewrite `c2→c3`, re-encode `c3→c4`, each a run of `U`'s own transitions within its
bound), `U` reaches `c4` from `c0` within `B = bDecode + bLookup + bRewrite + bReencode` steps — composed via the proved
additive `reachIn_add`.  Turns the entry-181 semantic `hstep` contract into a step-bounded simulation lemma; the
per-phase realizations are the remaining TM-engineering sockets. -/
theorem hstep_realized_with_overhead_frontier (U : ACC0NTM.NTM) (c0 c1 c2 c3 c4 : U.Config)
    (bD bL bR bE : ℕ)
    (hD : ACC0NTM.reachIn U bD c0 c1) (hL : ACC0NTM.reachIn U bL c1 c2)
    (hR : ACC0NTM.reachIn U bR c2 c3) (hE : ACC0NTM.reachIn U bE c3 c4) :
...
/-- **The combinatorial Chernoff bound (proved) — the per-input concentration's analytic heart.**  `binomial_tail_le`:
the binomial tail `∑_{j≤m} C(k,j)·a^j·b^{k−j} ≤ 2^k·a^m·b^{k−m}` (`b ≤ a`, `m ≤ k`), no measure theory.  In the `3/4`
regime (`a ≥ 3b`, `m ≈ k/2`) this is `exp(−Ω(k))` of `(a+b)^k` — the Chernoff decay.  (Corrects entry 208: the genuine
concentration is over a `≥3/4` supply, not all functions.) -/
theorem nw_binomial_tail_le_frontier {a b m k : ℕ} (hba : b ≤ a) (hmk : m ≤ k) :
...
/-- **Entry 345: the full `scanNat` run (PROVED).**  From `(0,0, encodeNatBits n ++ rest)`, `scanNat` runs `n+1` steps
to state `1` at head `n+1` — the verified cursor-advance over one encoded nat (tape-invariant induction on the
write-back invariant). -/
theorem nw_scanNat_run_frontier (n : ℕ) (rest : List Bool) :
    ∃ tp, ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM ACC0UniversalTMScanNat.scanNat) (n + 1)
...
/-- **Entry 351: the one-bit branch machine `branchBit` (PROVED).**  The first non-advancing machine in the table
build: at offset `h`, reading `true` routes to state `sTrue` and reading `false` routes to `sFalse`, leaving the tape
and head untouched — the decision atom for the rule-table scan-and-match.  Here is the `true` branch. -/
theorem nw_branchBit_run_true_frontier (s sTrue sFalse h : ℕ) (tp : List Bool)
    (hb : tp.getD h false = true) :
    ∃ tp', ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM (ACC0UniversalTMBranch.branchBit s sTrue sFalse)) 1
...
/-- **Entry 357: the fixed-distance walk `walkRightK` (PROVED).**  Unlike entry-334's single-state self-loop,
`walkRightK k s` walks the head right `k` cells through the distinct states `s, …, s+k`, ending in the controlled state
`s+k` at head `h+k`, preserving the tape — the gap-traversal piece a distant-region comparison chains between carrying a
bit and comparing. -/
theorem nw_walkRightK_run_frontier (k s h : ℕ) (tp : List Bool) :
    ∃ tp', ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM (ACC0UniversalTMWalkRightK.walkRightK k s)) k
...
/-- **Entry 359: the overwrite primitive `writeConst` (PROVED).**  The first machine in the table build to *change* a
tape cell rather than restore it — `writeConst b s s'` overwrites the head cell with the constant `b` (whatever it held)
and advances, reaching `(s', h+1, writeAt tp h b)` in one step.  The atom of the apply phase. -/
theorem nw_writeConst_run_frontier (b : Bool) (s s' h : ℕ) (tp : List Bool) :
    ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM (ACC0UniversalTMWriteConst.writeConst b s s')) 1
...
/-- **Entry 362: the fixed-distance left walk `walkLeftK` (PROVED).**  The left analog of `walkRightK` and the
return-trip primitive for the apply's block copy: `walkLeftK k s` walks the head left `k` cells through the states
`s, …, s+k`, ending in state `s+k` at head `h-k`, preserving the tape. -/
theorem nw_walkLeftK_run_frontier (k s h : ℕ) (tp : List Bool) :
    ∃ tp', ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM (ACC0UniversalTMWalkLeftK.walkLeftK k s)) k
        (s, h, tp) (s + k, h - k, tp') ∧ ∀ q, tp'.getD q false = tp.getD q false :=
...
/-- **Entry 364: the single-base copy step `copyStep` (PROVED).**  `copyBitReturn` repackaged with its internal states
fixed relative to one base `s` — one entry state `s`, one exit state `s+3m+3` — so a block copy can chain it.  Runs
`2m+2` steps from `(s, j, tp)` to `(s+3m+3, j+1, tp')`, copying cell `j` to `j+m` and returning the head to `j+1`. -/
theorem nw_copyStep_run_frontier (m s j : ℕ) (tp : List Bool) :
    ∃ tp', ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM (ACC0UniversalTMCopyStep.copyStep m s)) (2 * m + 2)
        (s, j, tp) (s + 3 * m + 3, j + 1, tp') ∧ tp'.getD (j + m) false = tp.getD j false ∧
      ∀ q, q ≠ j + m → tp'.getD q false = tp.getD q false :=
  ACC0UniversalTMCopyStep.copyStep_run m s j tp

/-- **Entry 365: the block copy `copyBlock` (PROVED).**  Loops `copyStep` `L` times to transplant a whole field — the
source block `[j, j+L)` to the destination `[j+m, j+m+L)`.  For `L ≤ m` (region-disjointness, so each iteration reads
the original source), runs `L*(2m+2)` steps with the destination mirroring the source and every cell before it
(including the whole source) unchanged. -/
theorem nw_copyBlock_run_frontier (m L s j : ℕ) (tp : List Bool) (hL : L ≤ m) :
    ∃ tp', ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM (ACC0UniversalTMCopyBlock.copyBlock m L s))
        (L * (2 * m + 2)) (s, j, tp) (s + L * (3 * m + 3), j + L, tp') ∧
...
/-- **Entry 367: the relocatable record scanner `scanTransFrom` (PROVED).**  `scanTrans` generalised to an arbitrary
state base and head offset: from state `base` at the start of a record (head `pre.length`), `scanTransFrom base` scans
the whole `encodeTransBits t` in `rs+ws+mv+5` steps to state `base+5`, head past the record — the loopable unit of the
rule-table traversal. -/
theorem nw_scanTransFrom_run_frontier (base : ℕ) (t : ACC0ConcreteNTM.TMTrans) (pre rest : List Bool) :
...
/-- **Entry 374: the list-preserving fixed-distance walk `walkRightK_run_eq` (PROVED).**  With all walked positions in
bounds (`h + k ≤ tp.length`), `walkRightK k s` walks `k` cells right returning the *identical* tape — each `scanBit`
step is the identity in bounds.  The walk the list-preserving `compareDistant` needs. -/
theorem nw_walkRightK_run_eq_frontier (k s h : ℕ) (tp : List Bool) (hbound : h + k ≤ tp.length) :
    ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM (ACC0UniversalTMWalkRightK.walkRightK k s)) k
      (s, h, tp) (s + k, h + k, tp) :=
...
/-- **Entry 377: the compare-and-return body `compareBitReturn` (PROVED), match case.**  Compares cell `j` with `j+m`
and, on equal cells (with `j+m+1 < tp.length`), reaches `E+m` at head `j+1` (the next source cell) leaving the tape
*identical* — the loop body of a cell-by-cell field comparison (on a mismatch it instead halts at `N`). -/
theorem nw_compareBitReturn_run_eq_frontier (m s sT0 sF0 E N j : ℕ) (tp : List Bool)
    (heq : tp.getD j false = tp.getD (j + m) false) (hbound : j + m + 1 < tp.length) :
    ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM (ACC0UniversalTMCompareBitReturn.compareBitReturn m s sT0 sF0 E N))
...
/-- **Entry 382: the key match no-match direction (state differs) `keyMatch_run_ne_state` (PROVED).**  If the state
fields differ, `keyMatch` reaches `noMatch` (the symbol phase, entered only at `matchSt`, is never run) — completing
(with entry 381 and the symbol-differ case) the per-rule decision: `fullMatch` iff keys equal, `noMatch` otherwise. -/
theorem nw_keyMatch_run_ne_state_frontier (m sT0 sF0 sCont matchSt noMatch bT0 bF0 fullMatch k j : ℕ) (tp : List Bool)
    (htrueA : ∀ i, i < k → tp.getD (j + i) false = true) (htrueB : ∀ i, i < k → tp.getD (j + m + i) false = true)
    (hdiff : tp.getD (j + k) false ≠ tp.getD (j + m + k) false) (hbound : j + m + k + 1 < tp.length) :
    ∃ N, ACC0NTM.reachIn (ACC0ConcreteNTM.toNTM
        (ACC0UniversalTMKeyMatch.keyMatch m sT0 sF0 sCont matchSt noMatch bT0 bF0 fullMatch)) N
      (sCont + m, j, tp) (noMatch, j + k + m + 1, tp) :=
...
/-- **Entry 384: sequential composition over the 3-symbol model `reachIn_seq3` (PROVED).**  Ports the entry-347 wiring
law to `concreteStep3`: a run of `M₁` then a run of `M₂` is a single run of `M₁ ++ M₂` (no disjointness needed) — the
composition the marker-based machines are assembled from. -/
theorem nw_reachIn_seq3_frontier (M₁ M₂ : ACC0UniversalTM3Sym.TMachine3) (a b : ℕ)
    (c d e : ACC0UniversalTM3Sym.CConfig3)
    (h1 : ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3 M₁) a c d)
    (h2 : ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3 M₂) b d e) :
...
/-- **Entry 389: anchor-and-carry `markCarry3_run_O` (PROVED).**  The marker comparison begins each cell-pair by
anchoring one operand: read the bit, remember its value in the control state, and overwrite the cell with the marker `M`.
Here the cell is `O`: the machine writes `M` and goes to `sO` (remembering `O`), head unchanged. -/
theorem nw_markCarry3_run_O_frontier (s sO sI sM j : ℕ) (tp : List ACC0UniversalTM3Sym.Sym3)
    (h : ACC0UniversalTM3Sym.readSym3 (s, j, tp) = ACC0UniversalTM3Sym.Sym3.O) :
    ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3 (ACC0UniversalTM3MarkCarry.markCarry3 s sO sI sM)) 1
...
/-- **Entry 391: double-write collapse `writeAt3_writeAt3` (PROVED).**  Writing twice at the same cell keeps only the
last write — the algebraic basis of the marker restore (anchor with `M`, then write the carried value back collapses to
a single write of that value). -/
theorem nw_writeAt3_writeAt3_frontier (tape : List ACC0UniversalTM3Sym.Sym3) (p : ℕ)
    (a b : ACC0UniversalTM3Sym.Sym3) :
    ACC0UniversalTM3Sym.writeAt3 (ACC0UniversalTM3Sym.writeAt3 tape p a) p b
...
/-- **Entry 394: the 3-symbol machine encoding `encodeMachineBits3_length` (PROVED).**  A machine `M` is encoded over
`Sym3` as its length (`I`-run + separator) then its transitions concatenated; its length is `(M.length + 1) +
(M.flatMap encodeTransBits3).length`.  The `Sym3` port of entry 338. -/
theorem nw_encodeMachineBits3_length_frontier (M : List ACC0ConcreteNTM.TMTrans) :
    (ACC0UniversalTM3EncMachine.encodeMachineBits3 M).length
...
/-- **Entry 414: the leftward single-cell copy `copyBitAtDistLeft3_run` (PROVED).**  The mirror of the rightward copy
(411) — the direction the apply phase needs, since the configuration sits to the *left* of the rule table.  Read the
source bit marker-free, walk left `d`, write it at `p-d`, walk back right the fixed `d` cells.  Copies `tp[p]` into cell
`p-d`, head back at `p`, source unchanged. -/
theorem nw_copyBitAtDistLeft3_run_frontier (s sOut d p : ℕ) (tp : List ACC0UniversalTM3Sym.Sym3)
    (hbit : tp.getD p ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.O ∨
      tp.getD p ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.I)
    (hd : 1 ≤ d) (hdp : d ≤ p) (hp : p < tp.length) :
    ∃ N, ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3 (ACC0UniversalTM3CopyBitLeft.copyBitAtDistLeft3 s sOut d)) N
      (s, p, tp) (sOut, p, ACC0UniversalTM3Sym.writeAt3 tp (p - d) (tp.getD p ACC0UniversalTM3Sym.Sym3.O)) :=
  ACC0UniversalTM3CopyBitLeft.copyBitAtDistLeft3_run s sOut d p tp hbit hd hdp hp

/-- **Entry 415: the leftward unary field copy `copyFieldLeft3_run` (PROVED).**  Loops the leftward single-cell copy (414)
over a whole unary field — the new-state copy in the apply direction, the leftward mirror of `copyField3` (412).  Copies
the field (`m` ones then `O` at `c`) to the destination region `c-d … c+m-d`; needs only `d ≤ c` (room to the left), not
`m < d`, since the destination is always behind the advancing source. -/
theorem nw_copyFieldLeft3_run_frontier (sDone d L s c m : ℕ) (tp : List ACC0UniversalTM3Sym.Sym3)
    (hmL : m < L) (hd : 1 ≤ d) (hdc : d ≤ c) (hbnd : c + m < tp.length)
    (hco : ∀ i, i < m → tp.getD (c + i) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.I)
    (hcs : tp.getD (c + m) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.O) :
...
/-- **Entry 430: the state-field update `clearStateField3_run` (PROVED).**  The complete simulated-state update: clear the
config's old state field (418), then transfer the matched rule's new-state field in (428), handling the length change.  The
two phases run on different tapes; the cross-tape obligations (home marker and the distant rule field survive clearing) are
discharged by the clearer-content lemmas (429).  Replaces the config state field with a copy of the rule's new state. -/
theorem nw_clearStateField3_run_frontier (s sMid found cont mid d' sDone L1 L2 home oldlen newlen : ℕ)
    (tp : List ACC0UniversalTM3Sym.Sym3)
    (hmark : tp.getD home ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hclean : ∀ j, j ≠ home → tp.getD j ACC0UniversalTM3Sym.Sym3.O ≠ ACC0UniversalTM3Sym.Sym3.M)
    ...
    ∃ N, ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3
        (ACC0UniversalTM3StateUpdate.clearStateField3 s sMid found cont mid d' sDone L1 L2)) N (s, home + 1, tp)
      (sDone, home + 1 + d' + newlen,
        ACC0UniversalTM3CopyFieldLeft.copyBlockLeft (ACC0UniversalTM3Walk.clearBlock tp (home + 1) oldlen)
          (home + 1 + d') d' newlen) :=
...
/-- **Entry 433: the cache-refresh navigation `cacheRefreshNav3_run` (PROVED).**  From inside the tape (right of the head
marker), skip the head marker (432) then navigate to the cache (431) — seeking past to the home marker and scanning the
state field to the cache cell `c+a+1`.  Tape identical. -/
theorem nw_cacheRefreshNav3_run_frontier (s found1 cont1 s2 found2 cont2 mid sOut hm e home a : ℕ)
    (tp : List ACC0UniversalTM3Sym.Sym3) (hhome_lt : home + 1 ≤ hm)
    ...
    (hbnd2 : home + 1 + a < tp.length) :
    ∃ N, ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3
        (ACC0UniversalTM3CacheNav.cacheRefreshNav3 s found1 cont1 s2 found2 cont2 mid sOut)) N (s, hm + e, tp)
      (sOut, home + 1 + a + 1, tp) :=
...
/-- **Entry 438: the move-direction dispatch (right entry) `moveHeadDispatch3_run_right` (PROVED).**  Both canonical move
phases (436, 437) under one machine; entered at the right-entry state it runs the right move, the unused left phase lifted
into the union.  Marker `p → p+1`, head returns to `home+1`. -/
theorem nw_moveHeadDispatch3_run_right_frontier
    (sR sR1 fR1 cR1 sR2 fR2 cR2 outR sL sL1 fL1 cL1 sL2 fL2 cL2 outL home p : ℕ) (tp : List ACC0UniversalTM3Sym.Sym3)
    (hhp : home < p) (hmarkHome : tp.getD home ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hmarkHead : tp.getD p ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hclean : ∀ j, home < j → j < p → tp.getD j ACC0UniversalTM3Sym.Sym3.O ≠ ACC0UniversalTM3Sym.Sym3.M)
    (hc : tp.getD (p + 1) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.O ∨
      tp.getD (p + 1) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.I) (hbnd : p + 1 < tp.length) :
    ∃ N, ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3
        (ACC0UniversalTM3MoveDispatch.moveHeadDispatch3 sR sR1 fR1 cR1 sR2 fR2 cR2 outR sL sL1 fL1 cL1 sL2 fL2 cL2 outL)) N
      (sR, p, tp) (outR, home + 1, ACC0UniversalTM3Sym.writeAt3 (ACC0UniversalTM3Sym.writeAt3 tp p
        (tp.getD (p + 1) ACC0UniversalTM3Sym.Sym3.O)) (p + 1) ACC0UniversalTM3Sym.Sym3.M) :=
  ACC0UniversalTM3MoveDispatch.moveHeadDispatch3_run_right sR sR1 fR1 cR1 sR2 fR2 cR2 outR sL sL1 fL1 cL1 sL2 fL2 cL2 outL
    home p tp hhp hmarkHome hmarkHead hclean hc hbnd

/-- **Entry 438 (cont.): the move-direction dispatch (left entry) `moveHeadDispatch3_run_left` (PROVED).**  Entered at the
left-entry state it runs the left move.  Marker `p → p-1`, head returns to `home+1`. -/
theorem nw_moveHeadDispatch3_run_left_frontier
    (sR sR1 fR1 cR1 sR2 fR2 cR2 outR sL sL1 fL1 cL1 sL2 fL2 cL2 outL home p : ℕ) (tp : List ACC0UniversalTM3Sym.Sym3)
    (hhp : home + 2 ≤ p) (hmarkHome : tp.getD home ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hmarkHead : tp.getD p ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hclean : ∀ j, home < j → j < p → tp.getD j ACC0UniversalTM3Sym.Sym3.O ≠ ACC0UniversalTM3Sym.Sym3.M)
    ...
    ∃ N, ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3
        (ACC0UniversalTM3MoveDispatch.moveHeadDispatch3 sR sR1 fR1 cR1 sR2 fR2 cR2 outR sL sL1 fL1 cL1 sL2 fL2 cL2 outL)) N
      (sL, p, tp) (outL, home + 1, ACC0UniversalTM3Sym.writeAt3 (ACC0UniversalTM3Sym.writeAt3 tp p
        (tp.getD (p - 1) ACC0UniversalTM3Sym.Sym3.O)) (p - 1) ACC0UniversalTM3Sym.Sym3.M) :=
  ACC0UniversalTM3MoveDispatch.moveHeadDispatch3_run_left sR sR1 fR1 cR1 sR2 fR2 cR2 outR sL sL1 fL1 cL1 sL2 fL2 cL2 outL
    home p tp hhp hmarkHome hmarkHead hclean hc hbnd

/-- **Entry 439: the home-to-home right move `moveRightFromHome3_run` (PROVED).**  A master-sequence phase that starts and
ends at the config home: seek from the home to the head marker (388), then advance the head right and return home (436).
The head marker advances `p → p+1`, head returns to `home+1`. -/
theorem nw_moveRightFromHome3_run_frontier (sStart sFound sCont s1 f1 c1 s2 f2 c2 out home p : ℕ)
    ...
    (hc : tp.getD (p + 1) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.O ∨
      tp.getD (p + 1) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.I) (hbnd : p + 1 < tp.length) :
    ∃ N, ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3
        (ACC0UniversalTM3MoveRightHome.moveRightFromHome3 sStart sFound sCont s1 f1 c1 s2 f2 c2 out)) N
...
/-- **Entry 446: the home-to-home state update `clearStateFieldHome3_run` (PROVED).**  The state update (430) made
home-to-home by appending a reset (408); the marker invariant is re-established on the transformed tape via the copier (445)
and clearer (429) content.  Replaces the config state field with the rule's new state and returns the head to `home+1`. -/
theorem nw_clearStateFieldHome3_run_frontier (s sMid found cont mid d' sDone L1 L2 rf rc rout home oldlen newlen : ℕ)
    (tp : List ACC0UniversalTM3Sym.Sym3)
    (hmark : tp.getD home ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hclean : ∀ j, j ≠ home → tp.getD j ACC0UniversalTM3Sym.Sym3.O ≠ ACC0UniversalTM3Sym.Sym3.M)
  ...
  ACC0UniversalTM3StateUpdateHome.clearStateFieldHome3_run s sMid found cont mid d' sDone L1 L2 rf rc rout home oldlen
    newlen tp hmark hclean hd' hdold hnd hL1 hL2 hbnd hcold hcoldsep hcnew hcnewsep

/-- **Entry 447: the home-to-home symbol write `writeSymHome3_run` (PROVED).**  Write the matched rule's symbol at the
simulated current cell as a home-to-home phase: seek to the marker (388), step onto the current cell, write `w` (`unmark3`),
skip the marker (432), reset to home (408).  Sets the current cell `p+1` to `w`, head returns to `home+1`. -/
theorem nw_writeSymHome3_run_frontier (a aF aC b c cF cC d dF dC out home p : ℕ) (w : ACC0UniversalTM3Sym.Sym3)
    (tp : List ACC0UniversalTM3Sym.Sym3) (hhp : home < p)
    (hmarkHome : tp.getD home ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hmarkHead : tp.getD p ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hclean : ∀ j, home < j → j < p → tp.getD j ACC0UniversalTM3Sym.Sym3.O ≠ ACC0UniversalTM3Sym.Sym3.M)
    ...
    ∃ N, ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3
        (ACC0UniversalTM3WriteSymHome.writeSymHome3 a aF aC b c cF cC d dF dC out w)) N (a, home + 1, tp)
...
/-- **Entry 474: the comparison shuttle's half-tandem move `advanceCrossRight_cont_run` (PROVED).**  Fuses cursor-advance
(472) and inter-cursor cross (473): advance the cursor one cell within its field, then cross right to the partner cursor —
the reusable iteration half of the two-cursor comparison loop. -/
theorem nw_advanceCrossRight_cont_run_frontier (s smid sCont sEnd crMid crFound crCont cp d : ℕ)
    (tp : List ACC0UniversalTM3Sym.Sym3) (hM : tp.getD cp ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hI : tp.getD (cp + 1) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.I)
    (hPartner : tp.getD (cp + 2 + d) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.M)
    (hgap : ∀ k, k < d → tp.getD (cp + 2 + k) ACC0UniversalTM3Sym.Sym3.O ≠ ACC0UniversalTM3Sym.Sym3.M)
    (hbound : cp + 2 + d < tp.length) :
    ∃ N, ACC0NTM.reachIn
      (ACC0UniversalTM3Sym.toNTM3 (ACC0UniversalTM3AdvanceCross.advanceCrossRight s smid sCont sEnd crMid crFound crCont)) N
      (s, cp, tp) (crFound, cp + 2 + d,
        ACC0UniversalTM3Sym.writeAt3 (ACC0UniversalTM3Sym.writeAt3 tp cp ACC0UniversalTM3Sym.Sym3.I)
...
/-- **Entry 482: a comparison mismatch verdict `endMismatchCR_run` (PROVED).**  Config field ends but the record continues
(`a < b`): advance the config cursor onto its separator (472 end-case), seek to the record cursor (388), advance it
(cont-case, leaving a cursor), then erase that cursor (`eraseMark3`) — restoring the tape and landing in the fail state. -/
theorem nw_endMismatchCR_run_frontier (s mid1 cont1 e1 m2 sc2 mid3 endM erEntry failSt : ℕ)
    (tp : List ACC0UniversalTM3Sym.Sym3) (cp g d : ℕ) (hg : 2 ≤ g)
    (hCfield : tp.getD (cp + d) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.I)
    (hCsep : tp.getD (cp + d + 1) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.O)
    (hRfield : tp.getD (cp + g + d) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.I)
    (hRcont : tp.getD (cp + g + d + 1) ACC0UniversalTM3Sym.Sym3.O = ACC0UniversalTM3Sym.Sym3.I)
    (hgap : ∀ j, cp + d < j → j < cp + g + d → tp.getD j ACC0UniversalTM3Sym.Sym3.O ≠ ACC0UniversalTM3Sym.Sym3.M)
    (hbound : cp + g + d + 1 < tp.length) :
    ∃ N, ACC0NTM.reachIn (ACC0UniversalTM3Sym.toNTM3
      (ACC0UniversalTM3EndMismatch.endMismatchCR s mid1 cont1 e1 m2 sc2 mid3 endM erEntry failSt)) N
...
/-- **Brick (AC⁰[p] repr): recursive faithful representation of AC⁰[p] circuits over `F_p` `reprP_eval` (PROVED).**  Every
`AC⁰[p]` circuit (binary AND/OR/NOT + `MOD_p` gates) is represented *exactly* by `reprP C` over `F_p`, with `eval (bv ∘ x)
(reprP C) = bv (ACC0Circuit.eval C x)` on all Boolean inputs, of degree `≤ reprDegP C`.  The `MOD_p` gate `[∑_{i∈S} xᵢ ≡ t]`
is the Fermat indicator `1 - (∑_{i∈S} Xᵢ - t)^{p-1}`.  This is the object the RS separation runs on; `MOD_q` (`q≠p`) having no
such representation is the lower bound's content. -/
theorem nw_reprP_eval_frontier {n p : ℕ} [Fact p.Prime] (C : ACC0CircuitModel.ACC0Circuit n)
    (x : Fin n → Bool) (h : ACC0CircuitReprP.ModpOnly p C) :
    MvPolynomial.eval (fun i => (ACC0CircuitRepr.bv (x i) : ZMod p)) (ACC0CircuitReprP.reprP p C)
      = ACC0CircuitRepr.bv (ACC0CircuitModel.eval C x) :=
  ACC0CircuitReprP.reprP_eval C x h

/-- **Brick (AC⁰[p] repr, cont.): the representation has degree at most `reprDegP` (PROVED).** -/
...
/-- **The universal NTM simulation has overhead exactly 1 (proved) — the socket's quantitative heart.**
nw_univ_simulates_exactly_frontier (PROVED): the universal NTM `univNTM ⟨M⟩` accepts `x` within `t` steps iff `M` does
— the *same* time bound, no per-step blowup (the interpreter step `univStep ⟨M⟩ = concreteStep M` lifted through
reachIn/acceptsWithin).  So the universal-simulation socket's feared "physical overhead" is `1`, free; what remains is
only the decode-from-input uniformity (the universal TM, Turing 1936) — formalization, not an obstruction (entry 296). -/
theorem nw_univ_simulates_exactly_frontier
    (M : ACC0ConcreteNTM.TMachine) (x : List Bool) (t : ℕ) :
    ACC0NTM.acceptsWithin (ACC0UniversalSimulationOverhead.univNTM (ACC0ConcreteNTM.machineEquiv M)) x t ↔
      ACC0NTM.acceptsWithin (ACC0ConcreteNTM.toNTM M) x t :=
...
/-- **Realization wiring: clock the universal machine to the bigger bound (proved).**
nw_uniformU_clocked_within_frontier (PROVED): the single fixed universal machine `uniformU decPair`, on the encoded
input, accepts within any `t' ≥ t` for which `M` accepts `x` within `t` — overhead-1 simulation (297) + budget
monotonicity.  So a `g`-time machine fits any `f ≥ g`: the "clock to the bigger bound" step of the hierarchy, free
given overhead 1.  `diag_in_big` thereby reduces to building the lazy-diagonal decider machine (entry 298). -/
theorem nw_uniformU_clocked_within_frontier
    (M : ACC0ConcreteNTM.TMachine) (x : List Bool) {t t' : ℕ}
    (htt' : t ≤ t') (hsim : ACC0NTM.acceptsWithin (ACC0ConcreteNTM.toNTM M) x t) :
    ACC0NTM.acceptsWithin (ACC0DecodeUniformity.uniformU ACC0DecodeUniformity.decPair)
      (ACC0DecodeUniformity.encPair (ACC0ConcreteNTM.machineEquiv M) x) t' :=
