...
`P ≠ NP` separation via this schema requires, what it costs, and what it evades — every clause
...
* **Barrier evasion** (`TraceRelativization` / `NaturalProofs`): `traceInv` is machine-dependent
  (evades relativization) and generically-sound measures are non-large (evade natural proofs).

`traceSchema_frontier` bundles the SATV-relative clauses; `traceSchema_evades_relativization`
is the SATV-free machine-dependence.  The single remaining open crux — a super-additive
generically-sound measure superpolynomial on SAT-decider traces — is precisely the `P` vs `NP`
problem itself, now bounded on every side that abstract theorems can bound it.
...
5. **Non-large** — every generically-sound measure evades the natural-proofs largeness condition.
...
theorem traceSchema_frontier (SATV : NPObs) :
    ...
    -- (5) non-large: evades natural proofs
...
/-- **The schema evades relativization** (SATV-free): `traceInv` is machine-dependent, so it is
not determined by the decided language — a relativizing measure could not distinguish the two
same-language machines it separates. -/
theorem traceSchema_evades_relativization :
