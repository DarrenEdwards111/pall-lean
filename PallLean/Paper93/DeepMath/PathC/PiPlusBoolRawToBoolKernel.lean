...
/-- The concrete paper-scale exact window: one representative from each of the
first 804 locality blocks, namely variables `0,3,6,...,3*803`. -/
noncomputable def paperScaleSpacedVar (k : Fin 804) : Fin ((2 : Nat) ^ 804) :=
  ⟨3 * k.val, by
    have hk : k.val < 804 := k.isLt
    have hsmall : 3 * k.val < 3 * 804 := by nlinarith
    have hbig : 3 * 804 < (2 : Nat) ^ 804 := by norm_num
    exact lt_trans hsmall hbig⟩

/-- The length-804 paper-scale derivative window with no two variables in the
same locality block. -/
noncomputable def paperScaleSpacedWindow : List (Fin ((2 : Nat) ^ 804)) :=
  List.ofFn paperScaleSpacedVar

/-- The spaced window has exactly the paper-scale strict derivative length. -/
theorem paperScaleSpacedWindow_length : paperScaleSpacedWindow.length = 804 := by
  rw [paperScaleSpacedWindow, List.length_ofFn]

/-- The spaced representative map is injective. -/
theorem paperScaleSpacedVar_injective : Function.Injective paperScaleSpacedVar := by
  intro a b h
  apply Fin.ext
  have hv := congrArg (fun x : Fin ((2 : Nat) ^ 804) => x.val) h
  simp [paperScaleSpacedVar] at hv
  omega

/-- Hence the spaced window has no duplicate variables. -/
theorem paperScaleSpacedWindow_nodup : paperScaleSpacedWindow.Nodup := by
  rw [paperScaleSpacedWindow]
  exact List.nodup_ofFn_ofInjective paperScaleSpacedVar_injective

/-- A nodup list filtered by a block assignment has length at most one when the
assignment is injective on the list. -/
...
/-- The Cook--Levin locality partition is injective on the spaced window: each
chosen variable lives in a distinct block. -/
theorem paperScaleSpacedWindow_partition_injOn (M : DTM)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ (2 : Nat) ^ 804) :
    Set.InjOn
      (cook_levin_compilation M ((2 : Nat) ^ 804) paperScale_ge_two htb hns).partition.assign
      {x | x ∈ paperScaleSpacedWindow} := by
  intro x hx y hy hxy
  have hxmem : x ∈ paperScaleSpacedWindow := hx
  have hymem : y ∈ paperScaleSpacedWindow := hy
  rw [paperScaleSpacedWindow] at hxmem hymem
  rw [List.mem_ofFn] at hxmem hymem
  rcases hxmem with ⟨a, rfl⟩
  rcases hymem with ⟨b, rfl⟩
  apply Fin.ext
  have hv := congrArg Fin.val hxy
  rw [cook_levin_assign, cook_levin_assign] at hv
  have hva : (paperScaleSpacedVar a).val / 3 = a.val := by
    simp [paperScaleSpacedVar]
  have hvb : (paperScaleSpacedVar b).val / 3 = b.val := by
    simp [paperScaleSpacedVar]
  rw [hva, hvb] at hv
  simp [paperScaleSpacedVar]
  omega

/-- Therefore the spaced window is block-admissible for the actual paper-scale
Cook--Levin locality partition. -/
theorem paperScaleSpacedWindow_isBlockAdmissible (M : DTM)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ (2 : Nat) ^ 804) :
    isBlockAdmissible
      (cook_levin_compilation M ((2 : Nat) ^ 804) paperScale_ge_two htb hns).partition
      paperScaleSpacedWindow := by
  constructor
  · exact paperScaleSpacedWindow_nodup
  · intro b
    exact nodup_filter_eq_length_le_one paperScaleSpacedWindow_nodup
      (paperScaleSpacedWindow_partition_injOn M htb hns) b


/-- The first spaced variable belongs to the paper-scale window. -/
theorem paperScaleSpacedWindow_zero_mem :
    paperScaleSpacedVar 0 ∈ paperScaleSpacedWindow := by
  rw [paperScaleSpacedWindow]
  rw [List.mem_ofFn]
  exact ⟨0, rfl⟩

/-- The nominal paper-scale logarithmic derivative budget is exactly `804`. -/
theorem paperScale_log_two_pow_804 : Nat.log 2 ((2 : Nat) ^ 804) = 804 := by
  norm_num

/-- In particular the paper-scale shift budget is large enough for the
quadratic Boolean residual multiplier. -/
theorem paperScale_log_two_pow_804_ge_two : 2 ≤ Nat.log 2 ((2 : Nat) ^ 804) := by
  norm_num

/-- Concrete paper-scale consequence of exact raw-to-Boolean kernel-disjointness:
the derivative of the actual compiled Cook--Levin polynomial along the explicit
804-variable admissible window `0,3,6,...,2409` must vanish.  Thus any nonzero
certificate for this derivative refutes the paper-scale kernel-disjoint seam. -/
theorem iterDerivList_paperScaleSpacedWindow_eq_zero_of_kernelDisjoint
    (M : DTM) (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ (2 : Nat) ^ 804)
    (hdisj : PaperScaleCookLevinRawToBoolSourceNPKernelDisjoint M htb hns) :
    iterDerivList paperScaleSpacedWindow
      (compiledPoly (cook_levin_compilation M ((2 : Nat) ^ 804) paperScale_ge_two htb hns)) = 0 := by
  unfold PaperScaleCookLevinRawToBoolSourceNPKernelDisjoint at hdisj
  have htarget := iterDerivList_eq_zero_of_disjoint_rawBlockedSpdpSubspace_kernel
    (paperScaleSpacedVar 0) paperScaleSpacedWindow
    (compiledPoly (cook_levin_compilation M ((2 : Nat) ^ 804) paperScale_ge_two htb hns))
    (cook_levin_compilation M ((2 : Nat) ^ 804) paperScale_ge_two htb hns).partition
    (Nat.log 2 ((2 : Nat) ^ 804))
    paperScaleSpacedWindow_zero_mem paperScale_log_two_pow_804_ge_two
    (paperScaleSpacedWindow_isBlockAdmissible M htb hns) ?_
  · exact htarget
  · rw [paperScaleSpacedWindow_length, paperScale_log_two_pow_804]
    exact hdisj
...
#print axioms paperScaleSpacedWindow_isBlockAdmissible
#print axioms iterDerivList_paperScaleSpacedWindow_eq_zero_of_kernelDisjoint
