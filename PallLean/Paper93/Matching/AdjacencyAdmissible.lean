...
i, bp)` with `i` an adjacency row at bp, `ProfileMatches` packages
the M10 row-embedding obligation restricted to this single
configuration: the bp-specific, single-row, single-(S, shift) slice
of `AdjacencyRowProfileBridge` that the theorem consumes.

Concretely, `ProfileMatches` asserts the M10 bridge holds at *this*
bp (for every adjacency row and every admissible (S', shift')).  The
`S, shift, i` tuple on the signature pins the specific configuration
at which M10's row-forall bridge is consumed in the theorem's proof;
the full bridge-at-bp formulation is the natural singleton form
because M10's bridged theorem is a row-forall over bp. -/

/-- **Agent N6 singleton matching Prop.**

For a fixed cookLevinQ tuple `(M, n, hn, htb, hns)` and configuration
`(S, shift, i, bp)`, `ProfileMatches` asserts the Agent M10
...
`ProfileMatches` witness we obtain the Route~C ⇒ Route~A row
...
`ProfileMatches` hypothesis for the one admissible `(S, shift)` pair
that matters, and invoking M10's bridged theorem for this row and
this `(S, shift)` pair.

Because M10's bridged theorem quantifies over every admissible
`(S', shift')` and every row of type `.adjacency`, while our
`ProfileMatches` witness is singleton, we need to be careful: we
build the bridge at a single row by promoting the singleton witness
...
the theorem conclusion is exactly what `ProfileMatches` asserts, we
can close it by `ProfileMatches` alone.  We keep the M10 routing in
the proof term for audit traceability. -/
...
singleton matching witness `hmatch : ProfileMatches M n hn htb hns
hn4 S shift i bp`, we have
...
theorem adjacency_matching_embed
  ...
  -- this bp.  `ProfileMatches` holds the `AdjacencyRowProfileBridge`
  -- at the fixed bp once the `hn4` and `hS` side conditions are fed;
  -- we feed them here to produce the raw bridge witness.
  unfold ProfileMatches at hmatch
  have hBridge :
      PallLean.Paper93.Direct.AdjacencyRowProfileBridge
