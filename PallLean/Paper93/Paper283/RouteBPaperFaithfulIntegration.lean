...
The finite-row row-closure package is discharged by
`routeBRicherConcreteNPPrependedMultilinearRows_spdpRowClosurePackage`; the
only remaining SPDP-side input here is kernel compatibility for that selected
finite-row projection. -/
theorem routeBPerInstanceCertificate_of_prependedConcreteNPMultilinearTail_kernelCompatibility_endpointActive_booleanZeroNormalForms_deltaEqRateKappa
    {N d : Nat}
    (M : DTM) (n : Nat) (hn : n >= 2 ^ 804) (hn2 : n >= 2)
    (htb : M.timeBound <= 4) (hns : M.numStates <= n)
    (hn4 : n >= 4)
  ...
  routeBPerInstanceCertificate_of_prependedConcreteNP_kernelCompatibility_endpointActive_booleanZeroNormalForms_deltaEqRateKappa
    (N := N) (d := d)
    M n hn hn2 htb hns hn4
    alpha beta alpha0 kappa gadgetN G chi Phi
    (routeBRicherMultilinearTailRows M n hn2 htb hns)
    hN hrowCount heta htheta halpha halpha0 hkappa hgadgetN
    hzeroNormalForms hzeroBudget hactiveFrontier
    (routeBRicherConcreteNPPrependedMultilinearRows_spdpRowClosurePackage
      M n hn2 htb hns)
    hker

/-- Paper-faithful Route B integration specialized to the concrete
multilinear finite tail, with the remaining SPDP input stated as projection
descent rather than kernel compatibility.

The selected multilinear tail closes row-closure internally; projection
descent supplies the map-preimage SPDP surface directly. -/
theorem routeBPerInstanceCertificate_of_prependedConcreteNPMultilinearTail_projectionDescent_endpointActive_booleanZeroNormalForms_deltaEqRateKappa
    {N d : Nat}
    (M : DTM) (n : Nat) (hn : n >= 2 ^ 804) (hn2 : n >= 2)
    (htb : M.timeBound <= 4) (hns : M.numStates <= n)
    (hn4 : n >= 4)
