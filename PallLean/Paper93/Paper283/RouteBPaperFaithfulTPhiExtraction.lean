...
/-- Source-coordinate literal interface-profile local-basis data for strict
`TΦ`.

This is the most concrete paper-faithful Lemma 31 target after strict
first-of-block restriction: for each selected `ConstraintType` interface
profile it supplies an explicit bounded source-coordinate local basis, and the
selected restricted Cook-Levin derivative row lies in the span of that basis.
The ambient strict theorem is obtained only by rename transport. -/
structure RouteBPaperFaithfulTPhiStrictSourceConstraintTypeInterfaceProfileData
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n) where
  sourceLocalDim : ℕ
  sourceLocalDim_le :
  ...
  sourceLocalBasis_card_le : ∀ ρ, (sourceLocalBasis ρ).card ≤ sourceLocalDim
  profileOfCanonicalWindow :
    ∀ w : PallLean.Paper93.Window
        (RouteBPaperFaithfulTPhiStrictBlockIdx n)
        RouteBPaperFaithfulTPhiStrictLocalOp
      ...
      RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
        ConstraintType (Nat.log 2 n)
  canonicalSourceRow_mem_constraintTypeProfileSpan :
    ∀ (S' : List (Fin (n / 3)))
      (shift : MvPolynomial (Fin (n / 3)) ℚ)
      (α : Fin n →₀ ℕ)
      (hSlen : S'.length = Nat.log 2 n)
...
coordinates. -/
noncomputable def routeBPaperFaithfulTPhi_strictRenamedSourceLocalBasis
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    ...
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n)) :
    Finset (MvPolynomial (Fin n) ℚ) :=
  (D.sourceLocalBasis ρ).image
    (MvPolynomial.rename (cookLevinStrictFOBFlatMap n))

/-- Source-coordinate explicit local-basis data gives the ambient strict
`ConstraintType` interface-profile data by literally renaming the supplied
source bases.

This is a direct basis-level transport, not a common-span argument: the selected
source row is first placed in the selected source basis span, then the checked
strict `TΦ` row identity rewrites the ambient row as the rename of that source
row. -/
noncomputable def routeBPaperFaithfulTPhi_strictConstraintTypeInterfaceProfileData_of_sourceInterfaceProfileData_direct
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceConstraintTypeInterfaceProfileData
      M n hn2 htb hns) :
  ...
  localDim_le := D.sourceLocalDim_le
  localBasis := routeBPaperFaithfulTPhi_strictRenamedSourceLocalBasis D
  localBasis_card_le := by
    intro ρ
    exact (Finset.card_image_le.trans (D.sourceLocalBasis_card_le ρ))
  profileOfCanonicalWindow := D.profileOfCanonicalWindow
  canonicalRangeRow_mem_constraintTypeProfileSpan := by
    ...
    have hsrc : srcRow ∈
        Submodule.span ℚ
          (↑(D.sourceLocalBasis ρ) : Set (MvPolynomial (Fin (n / 3)) ℚ)) := by
      simpa [srcRow, ρ] using
        D.canonicalSourceRow_mem_constraintTypeProfileSpan
          S' shift α hSlen hshiftDegree hshiftVars hadm hrow
    have hmap :
        MvPolynomial.rename (cookLevinStrictFOBFlatMap n) srcRow ∈
    ...
    have hspan :
        Submodule.map (MvPolynomial.rename (cookLevinStrictFOBFlatMap n)).toLinearMap
            (Submodule.span ℚ
              (↑(D.sourceLocalBasis ρ) : Set (MvPolynomial (Fin (n / 3)) ℚ))) =
          Submodule.span ℚ
            (↑(routeBPaperFaithfulTPhi_strictRenamedSourceLocalBasis D ρ) :
              Set (MvPolynomial (Fin n) ℚ)) := by
      rw [Submodule.map_span]
      apply congrArg (Submodule.span ℚ)
      ext p
      simp [routeBPaperFaithfulTPhi_strictRenamedSourceLocalBasis]
    have hroweq :
        routeBPaperFaithfulTPhiStrictCanonicalDerivativeRow
            M n hn2 htb hns S' shift =
...
/-- Source local-basis data instantiates source selected profile-subspace data
by taking each source `V_h` to be the span of its source local basis. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceProfileSubspaceData_of_sourceInterfaceProfileData
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceConstraintTypeInterfaceProfileData
      M n hn2 htb hns) :
  ...
  sourceProfileSubspace_finite := by
    ...
    exact Module.Finite.span_of_finite ℚ
      (Finset.finite_toSet (D.sourceLocalBasis ρ))
  sourceProfileSubspace_finrank_le := by
    intro ρ
    exact
      (finrank_span_finset_le_card (D.sourceLocalBasis ρ)).trans
        ((D.sourceLocalBasis_card_le ρ).trans D.sourceLocalDim_le)
  profileOfCanonicalWindow := D.profileOfCanonicalWindow
...
/-- Source literal local-basis data gives ambient strict selected `V_h` data.

This is just composition of the two checked adapters: source local bases first
instantiate source selected subspaces, then rename transports those subspaces to
the ambient strict first-of-block coordinates. It deliberately does not infer a
selected profile from an ambient `iSup` membership and does not use a common
ambient span. -/
noncomputable def routeBPaperFaithfulTPhi_strictConstraintTypeProfileSubspaceData_of_sourceInterfaceProfileData
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceConstraintTypeInterfaceProfileData
      M n hn2 htb hns) :
...
/-- Paper-faithful finite local-monoid normal-form data for witnessed strict
`TΦ` Leibniz words.

This is one level more concrete than `WitnessedLeibnizLocalTypeMaps`: the local
type alphabet for each selected canonical-window profile is a finite local
monoid in the sense of paper §9.3, and the label is the shortlex normal form of
the actual local update word attached to the bounded Leibniz distribution
witness.

The membership field remains the real mathematical obligation: prove that the
projected shifted Leibniz word lies in the span of the basis for its own finite
local normal form.  No selected post-span collapse, common-span shortcut, or
single derivative-count histogram is introduced here. -/
structure RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizLocalMonoidNormalForms
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n) where
  profileOfCanonicalWindow :
    ∀ w : PallLean.Paper93.Window
  ...
  sourceNormalFormFintype :
    ∀ ρ, Fintype (sourceNormalForm ρ)
  sourceNormalFormMonoid :
    ∀ ρ, Monoid (sourceNormalForm ρ)
  sourceNormalFormDecidableEq :
    ∀ ρ, DecidableEq (sourceNormalForm ρ)
  sourceGenerators :
    ∀ ρ, List (sourceNormalForm ρ)
  sourceLocalDim :
    RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
  ...
  leibnizWitnessWord :
    ∀ (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ...
      (_hlen : ∑ i : Fin ((cookLevinFactorList M n hn2 htb hns).length),
          (d i).length ≤ S'.length),
        List (sourceNormalForm ρ)
  leibnizWitness_mem_normalFormSpace :
    ∀ (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
          ConstraintType (Nat.log 2 n))
      (S' : List (Fin (n / 3))) (hS : S'.length ≤ Nat.log 2 n)
      (shift : MvPolynomial (Fin (n / 3)) ℚ)
...
The seed is the explicit local basis for the empty witnessed generator word.
This is not a common/global span: it is a distinguished zero-letter local
component, used only when the paper's `Σ*` normal form has no letters. -/
noncomputable def routeBPaperFaithfulTPhi_seededFoldedUnion
    {α β : Type} [DecidableEq β]
    (w : List α) (B₀ : Finset β) (B : α → Finset β) : Finset β :=
  w.foldr (fun g acc => B g ∪ acc) B₀

/-- Membership in the empty-word seed injects into the seeded fold. -/
theorem routeBPaperFaithfulTPhi_mem_seededFoldedUnion_of_seed_mem
    {α β : Type} [DecidableEq β]
    (w : List α) (B₀ : Finset β) (B : α → Finset β)
    {x : β} (hx : x ∈ B₀) :
    x ∈ routeBPaperFaithfulTPhi_seededFoldedUnion w B₀ B := by
  induction w with
  | nil => simpa [routeBPaperFaithfulTPhi_seededFoldedUnion] using hx
  | cons g rest ih =>
      simp only [routeBPaperFaithfulTPhi_seededFoldedUnion, List.foldr_cons,
        Finset.mem_union]
      exact Or.inr ih

/-- Membership in one letter basis injects into the seeded fold. -/
theorem routeBPaperFaithfulTPhi_mem_seededFoldedUnion_of_mem
    {α β : Type} [DecidableEq β]
    (w : List α) (B₀ : Finset β) (B : α → Finset β)
    {g : α} (hg : g ∈ w) {x : β} (hx : x ∈ B g) :
    x ∈ routeBPaperFaithfulTPhi_seededFoldedUnion w B₀ B := by
...
/-- Attached-word seeded fold.  The basis may depend on the proof that a letter
occurs in the witnessed word. -/
noncomputable def routeBPaperFaithfulTPhi_seededAttachFoldedUnion
    {α β : Type} [DecidableEq β]
    (w : List α) (B₀ : Finset β) (B : {g : α // g ∈ w} → Finset β) :
    Finset β :=
  routeBPaperFaithfulTPhi_seededFoldedUnion w.attach B₀ B

/-- Membership in the empty-word seed injects into the seeded attached fold.
This is the paper-faithful zero-letter case; no generator letter is fabricated. -/
theorem routeBPaperFaithfulTPhi_mem_seededAttachFoldedUnion_of_seed_mem
    {α β : Type} [DecidableEq β]
    (w : List α) (B₀ : Finset β) (B : {g : α // g ∈ w} → Finset β)
    {x : β} (hx : x ∈ B₀) :
    x ∈ routeBPaperFaithfulTPhi_seededAttachFoldedUnion w B₀ B := by
  exact routeBPaperFaithfulTPhi_mem_seededFoldedUnion_of_seed_mem w.attach B₀ B hx

/-- A row in one certified attached-letter basis injects into the seeded
attached fold.  The seed does not weaken the nonempty case: the row still enters
through the exact witnessed letter component. -/
theorem routeBPaperFaithfulTPhi_mem_seededAttachFoldedUnion_of_letter_mem
    {α β : Type} [DecidableEq β]
    (w : List α) (B₀ : Finset β) (B : {g : α // g ∈ w} → Finset β)
    (g : {g : α // g ∈ w}) {x : β} (hx : x ∈ B g) :
    x ∈ routeBPaperFaithfulTPhi_seededAttachFoldedUnion w B₀ B := by
...
theorem routeBPaperFaithfulTPhi_span_seededAttachFoldedUnion_mem_of_seed_mem
    ...
    {x : V} (hx : x ∈ B₀) :
    x ∈ Submodule.span ℚ
      (↑(routeBPaperFaithfulTPhi_seededAttachFoldedUnion w B₀ B) : Set V) := by
  exact Submodule.subset_span
    (routeBPaperFaithfulTPhi_mem_seededAttachFoldedUnion_of_seed_mem w B₀ B hx)

/-- Letter membership gives span membership in the whole seeded attached basis.
This is the nonempty witnessed-letter insertion rule, still local to one
certified generator component. -/
theorem routeBPaperFaithfulTPhi_span_seededAttachFoldedUnion_mem_of_letter_mem
    {α V : Type} [DecidableEq V] [AddCommMonoid V] [Module ℚ V]
    (w : List α) (B₀ : Finset V) (B : {g : α // g ∈ w} → Finset V)
    (g : {g : α // g ∈ w}) {x : V} (hx : x ∈ B g) :
    x ∈ Submodule.span ℚ
      (↑(routeBPaperFaithfulTPhi_seededAttachFoldedUnion w B₀ B) : Set V) := by
  exact Submodule.subset_span
    (routeBPaperFaithfulTPhi_mem_seededAttachFoldedUnion_of_letter_mem w B₀ B g hx)

/-- Cardinality of a seeded folded union is bounded by the seed size plus
word length times a uniform per-letter budget.

This is the arithmetic form of the empty/nonempty `Σ*` split: the zero-letter
component is counted once, and the actual witnessed word letters are counted
separately. -/
theorem routeBPaperFaithfulTPhi_seededFoldedUnion_card_le_seed_add_length_mul
    {α β : Type} [DecidableEq β]
    (w : List α) (B₀ : Finset β) (B : α → Finset β) (C : ℕ)
    (hB : ∀ g ∈ w, (B g).card ≤ C) :
    (routeBPaperFaithfulTPhi_seededFoldedUnion w B₀ B).card ≤
      B₀.card + w.length * C := by
  induction w with
  | nil =>
      simp [routeBPaperFaithfulTPhi_seededFoldedUnion]
  | cons g rest ih =>
      have hg : (B g).card ≤ C := hB g (by simp)
      have hrest : ∀ x ∈ rest, (B x).card ≤ C := by
        intro x hx
      ...
      have hih := ih hrest
      calc
        (routeBPaperFaithfulTPhi_seededFoldedUnion (g :: rest) B₀ B).card
            = (B g ∪ routeBPaperFaithfulTPhi_seededFoldedUnion rest B₀ B).card := by
              simp [routeBPaperFaithfulTPhi_seededFoldedUnion]
        _ ≤ (B g).card + (routeBPaperFaithfulTPhi_seededFoldedUnion rest B₀ B).card :=
          Finset.card_union_le _ _
        _ ≤ C + (B₀.card + rest.length * C) := Nat.add_le_add hg hih
        _ = B₀.card + (g :: rest).length * C := by
          simp [Nat.succ_mul, Nat.add_comm, Nat.add_left_comm]

/-- Attached version of the seeded cardinality bound. -/
theorem routeBPaperFaithfulTPhi_seededAttachFoldedUnion_card_le_seed_add_length_mul
    {α β : Type} [DecidableEq β]
    (w : List α) (B₀ : Finset β) (B : {g : α // g ∈ w} → Finset β) (C : ℕ)
    (hB : ∀ g, (B g).card ≤ C) :
    (routeBPaperFaithfulTPhi_seededAttachFoldedUnion w B₀ B).card ≤
      B₀.card + w.length * C := by
  simpa [routeBPaperFaithfulTPhi_seededAttachFoldedUnion, List.length_attach] using
    (routeBPaperFaithfulTPhi_seededFoldedUnion_card_le_seed_add_length_mul
      w.attach B₀ B C (by intro g _hg; exact hB g))

/-- If the seed and every letter basis lie in a target submodule, then the span
of the seeded folded basis lies in that target. -/
theorem routeBPaperFaithfulTPhi_span_seededFoldedUnion_le_of_seed_and_forall_mem
    {α V : Type} [DecidableEq V] [AddCommMonoid V] [Module ℚ V]
    (w : List α) (B₀ : Finset V) (B : α → Finset V) (U : Submodule ℚ V)
    (hseed : ↑B₀ ⊆ (U : Set V))
    (hB : ∀ g ∈ w, ↑(B g) ⊆ (U : Set V)) :
    Submodule.span ℚ
      (↑(routeBPaperFaithfulTPhi_seededFoldedUnion w B₀ B) : Set V) ≤ U := by
  refine Submodule.span_le.mpr ?_
  intro x hx
  induction w with
  | nil =>
      simpa [routeBPaperFaithfulTPhi_seededFoldedUnion] using hseed hx
  | cons g rest ih =>
      change x ∈ B g ∪ routeBPaperFaithfulTPhi_seededFoldedUnion rest B₀ B at hx
      simp only [Finset.mem_union] at hx
      rcases hx with hx | hx
      · exact hB g (by simp) hx
      · exact ih (by intro y hy; exact hB y (by simp [hy])) hx

/-- Attached dependent version of the seeded span bound. -/
theorem routeBPaperFaithfulTPhi_span_seededAttachFoldedUnion_le_of_seed_and_forall_mem
    {α V : Type} [DecidableEq V] [AddCommMonoid V] [Module ℚ V]
    (w : List α) (B₀ : Finset V) (B : {g : α // g ∈ w} → Finset V)
    (U : Submodule ℚ V)
    (hseed : ↑B₀ ⊆ (U : Set V))
    (hB : ∀ g, ↑(B g) ⊆ (U : Set V)) :
    Submodule.span ℚ
      (↑(routeBPaperFaithfulTPhi_seededAttachFoldedUnion w B₀ B) : Set V) ≤ U := by
...
/-- Seeded version of the witnessed `NFOfWord` generator-letter basis.

The extra seed is the local component for the empty generator word.  This is the
faithful `Σ*` boundary case: if the witnessed normal form is empty, the row must
be proved in this explicit zero-letter component rather than by inventing a
letter or collapsing to a common span. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n))
    ...
    (d : Fin ((cookLevinFactorList M n hn2 htb hns).length) →
      List (Fin (n / 3))) :
    Finset (MvPolynomial (Fin (n / 3)) ℚ) :=
  routeBPaperFaithfulTPhi_seededAttachFoldedUnion
    (routeBPaperFaithfulTPhi_strictSourceLeibnizTraceNFOfWord
      M n hn2 htb hns d)
    (emptyBasis ρ)
    (fun g =>
...
/-- The seeded witnessed `NFOfWord` basis is bounded by the empty-word seed
budget plus the actual normal-form length times the per-letter budget. -/
theorem routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis_card_le_seed_add_length_mul
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n))
    ...
    (routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
      (M := M) (n := n) (hn2 := hn2) (htb := htb) (hns := hns)
      ρ emptyBasis letterBasis d).card ≤
      (emptyBasis ρ).card +
        (routeBPaperFaithfulTPhi_strictSourceLeibnizTraceNFOfWord
          M n hn2 htb hns d).length * C := by
  classical
  unfold routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
  exact routeBPaperFaithfulTPhi_seededAttachFoldedUnion_card_le_seed_add_length_mul
    (routeBPaperFaithfulTPhi_strictSourceLeibnizTraceNFOfWord
      M n hn2 htb hns d)
    (emptyBasis ρ)
    (fun g =>
...
/-- Discharge form for the seeded `NFOfWord` cardinality field.  The seed and
letter budgets remain explicit and separate. -/
theorem routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis_card_le_of_seed_length_budget
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n))
    ...
    (routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
      (M := M) (n := n) (hn2 := hn2) (htb := htb) (hns := hns)
      ρ emptyBasis letterBasis d).card ≤ sourceLocalDim ρ := by
  exact
    (routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis_card_le_seed_add_length_mul
      (M := M) (n := n) (hn2 := hn2) (htb := htb) (hns := hns)
      ρ emptyBasis letterBasis C hletter d).trans hbudget

/-- Empty-word row insertion for the seeded witnessed `NFOfWord` basis. -/
theorem routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis_span_mem_of_empty_mem
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n))
  ...
  unfold routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
  exact routeBPaperFaithfulTPhi_span_seededAttachFoldedUnion_mem_of_seed_mem
    (routeBPaperFaithfulTPhi_strictSourceLeibnizTraceNFOfWord
      M n hn2 htb hns d)
    (emptyBasis ρ)
    (fun g =>
...
/-- Certified-letter row insertion for the seeded witnessed `NFOfWord` basis. -/
theorem routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis_span_mem_of_letter_mem
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n))
    ...
    x ∈ Submodule.span ℚ
      (↑(routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
        ...
        ρ emptyBasis letterBasis d) : Set (MvPolynomial (Fin (n / 3)) ℚ)) := by
  unfold routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
  exact routeBPaperFaithfulTPhi_span_seededAttachFoldedUnion_mem_of_letter_mem
    (routeBPaperFaithfulTPhi_strictSourceLeibnizTraceNFOfWord
      M n hn2 htb hns d)
    (emptyBasis ρ)
    (fun y =>
...
The point of the statement is the quantification over `g ∈ NFOfWord ... d`.
The bridge is discharged by actual witnessed letters only; it does not replace
those letters by a common span or silently identify `NFOfWord` with total `NF`. -/
theorem routeBPaperFaithfulTPhi_strictSource_NFOfWordGeneratorBasis_span_le_totalNormalFormBasis_of_letterwise_mem
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n))
  ...
  classical
  unfold routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordGeneratorLetterBasis
  apply routeBPaperFaithfulTPhi_span_foldedUnion_le_of_forall_mem
  intro g _hg x hx
  exact hletter g.1 g.2 hx

/-- Concrete sufficient condition for the seeded `NFOfWord → total NF` transfer.
The empty-word seed and the witnessed letters are transferred separately; this
is the faithful zero-letter/nonzero-letter bridge. -/
theorem routeBPaperFaithfulTPhi_strictSource_seededNFOfWordGeneratorBasis_span_le_totalNormalFormBasis_of_seed_and_letterwise_mem
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n))
    ...
    Submodule.span ℚ
        (↑(routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
          (M := M) (n := n) (hn2 := hn2) (htb := htb) (hns := hns)
          ρ emptyBasis letterBasis d) : Set (MvPolynomial (Fin (n / 3)) ℚ)) ≤
      Submodule.span ℚ
        (↑(routeBPaperFaithfulTPhi_strictSourceTraceNormalFormLetterBasis
          (M := M) (n := n) (hn2 := hn2) (htb := htb) (hns := hns)
          ρ
  ...
  classical
  unfold routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
  apply routeBPaperFaithfulTPhi_span_seededAttachFoldedUnion_le_of_seed_and_forall_mem
  · exact hseed
  · intro g x hx
    exact hletter g.1 g.2 hx
...
/-- Constructor for the explicit `NFOfWord` transfer-data layer from the
standalone generator-basis data plus honest total-normal-form obligations.

The transfer input is deliberately letterwise over the witnessed `NFOfWord`;
the constructor merely packages it into the span inclusion required by the
legacy downstream route. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordGeneratorBasisTransferData_of_generatorBasisData_and_letterwiseTotalTransfer
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordGeneratorBasisData
      M n hn2 htb hns)
    (htotalCard :
      ∀ (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
            ConstraintType (Nat.log 2 n))
  ...
  leibnizTraceWord_mem_NFOfWordGeneratorBasis :=
    D.leibnizTraceWord_mem_NFOfWordGeneratorBasis
  sourceTotalNormalFormGeneratorBasis_card_le := htotalCard
...
/-- Budgeted witnessed `NFOfWord` generator-basis maps with the explicit
transfer bridge required by the legacy downstream total-normal-form route.

This is intentionally a separate layer: the Cook--Levin row remains proved in
the exact witnessed `NFOfWord` generator-letter span, while the compatibility
with total `NF` is recorded as an explicit span inclusion and separate total
finite-endomorphism budget. -/
structure RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n) extends
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisMaps
      M n hn2 htb hns where
...
/-- End-to-end constructor from witnessed `NFOfWord` local-basis maps to the
ordinary exact-`NFOfWord` transfer surface.

After the empty-seed specialization, this is the leanest paper-faithful final
path: the row is extracted from the witnessed local type into the exact
`NFOfWord` generator-letter span, and the only remaining obligations are the
universal generator-basis budgets and explicit total-normal-form transfer.  No
seed `+1` budget is required here. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps_of_budgetedLocalBasisMaps_and_transferBudgets
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns)
    ...
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps
      M n hn2 htb hns where
  toRouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisMaps :=
    routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisMaps_of_budgetedLocalBasisMaps_and_generatorBudgets
      M n hn2 htb hns D hNFCard hNFProfileBudget
  sourceTotalNormalFormGeneratorBasis_card_le := htotalCard
  sourceTotalFiniteEndBudget_le := htotalBudget
  nfOfWordGeneratorBasis_span_le_totalNormalFormBasis := by
    intro ρ d
...
/-- The certified generator alphabet profile budget follows from the total
finite-endomorphism profile budget, since the generator alphabet is a subtype of
the total finite endomorphism type. -/
theorem routeBPaperFaithfulTPhi_strictSource_generatorProfileBudget_le_of_totalFiniteEndBudget
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (sourceLocalDim :
      RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
  ...
  have hsub :
    ...
    exact Fintype.card_subtype_le
      (fun g : RouteBPaperFaithfulTPhiFiniteEnd
          (RouteBPaperFaithfulTPhiStrictSourceLeibnizTraceState
            M n hn2 htb hns (Nat.log 2 n)) =>
        g ∈ routeBPaperFaithfulTPhi_strictSourceLeibnizTraceGenerators
          M n hn2 htb hns (Nat.log 2 n))
  exact (Nat.mul_le_mul_right (sourceLocalDim ρ) hsub).trans (htotalBudget ρ)

/-- Direct exact-`NFOfWord` transfer constructor where the total-normal-form
cardinality field is discharged from a per-letter budget and the finite-end
normal-form length bound.

This removes the raw `htotalCard` obligation: it is enough to budget each
certified generator-letter basis by `C` and prove `(card finiteEnd + 1) * C`
fits in the selected local dimension. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps_of_budgetedLocalBasisMaps_and_perLetterTransferBudgets
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns)
    ...
    (hletterCard :
      ∀ (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
        ...
        (g : { g : RouteBPaperFaithfulTPhiFiniteEnd
            ...
            g ∈ routeBPaperFaithfulTPhi_strictSourceLeibnizTraceGenerators
              M n hn2 htb hns (Nat.log 2 n) }),
          (D.sourceGeneratorLetterBasis ρ g).card ≤ C)
    (hNFCard :
      ∀ (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
            ConstraintType (Nat.log 2 n))
        (d : Fin ((cookLevinFactorList M n hn2 htb hns).length) →
    ...
    (htotalCardBudget :
      ∀ ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
        ...
        (Fintype.card (RouteBPaperFaithfulTPhiFiniteEnd
          (RouteBPaperFaithfulTPhiStrictSourceLeibnizTraceState
            M n hn2 htb hns (Nat.log 2 n))) + 1) * C ≤ D.sourceLocalDim ρ)
    (htotalBudget :
      ∀ ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
          ConstraintType (Nat.log 2 n),
        Fintype.card
    ...
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps
      M n hn2 htb hns :=
  routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps_of_budgetedLocalBasisMaps_and_transferBudgets
    M n hn2 htb hns D hNFCard hNFProfileBudget
    (by
      intro ρ ν
      exact
        routeBPaperFaithfulTPhi_strictSourceTotalNormalFormGeneratorLetterBasis_card_le_of_finiteEnd_budget
          (M := M) (n := n) (hn2 := hn2) (htb := htb) (hns := hns)
          ρ D.sourceGeneratorLetterBasis D.sourceLocalDim C
          (hletterCard ρ) ν (htotalCardBudget ρ))
    htotalBudget hletterTransfer

/-- Direct exact-`NFOfWord` transfer constructor with both profile budget and
total-normal-form cardinality derived from total/per-letter budgets.

This is slimmer than
`..._of_budgetedLocalBasisMaps_and_perLetterTransferBudgets`: the generator
profile budget is obtained from the total finite-endomorphism budget by subtype
cardinality. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps_of_budgetedLocalBasisMaps_and_totalPerLetterBudgets
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns)
    ...
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps
      M n hn2 htb hns :=
  routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps_of_budgetedLocalBasisMaps_and_perLetterTransferBudgets
    M n hn2 htb hns D C hletterCard hNFCard
    (routeBPaperFaithfulTPhi_strictSource_generatorProfileBudget_le_of_totalFiniteEndBudget
      M n hn2 htb hns D.sourceLocalDim htotalBudget)
    htotalCardBudget htotalBudget hletterTransfer

/-- Direct exact-`NFOfWord` transfer constructor where the exact `NFOfWord`
cardinality field is discharged from a per-letter budget and an explicit
normal-form-length budget.

This removes the raw `hNFCard` obligation: it is enough to budget each certified
generator-letter basis by `C`, and prove the actual witnessed `NFOfWord` length
times `C` fits in the selected local dimension for every `d`. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps_of_budgetedLocalBasisMaps_and_lengthTotalBudgets
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns)
    ...
    (hNFLengthBudget :
      ∀ (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
        ...
        (d : Fin ((cookLevinFactorList M n hn2 htb hns).length) →
          List (Fin (n / 3))),
        (routeBPaperFaithfulTPhi_strictSourceLeibnizTraceNFOfWord
          M n hn2 htb hns d).length * C ≤ D.sourceLocalDim ρ)
    (htotalCardBudget :
      ∀ ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
          ConstraintType (Nat.log 2 n),
        (Fintype.card (RouteBPaperFaithfulTPhiFiniteEnd
    ...
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps
      M n hn2 htb hns :=
  routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps_of_budgetedLocalBasisMaps_and_totalPerLetterBudgets
    M n hn2 htb hns D C hletterCard
    (by
      intro ρ d
      exact
        routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordGeneratorLetterBasis_card_le_of_length_budget
          (M := M) (n := n) (hn2 := hn2) (htb := htb) (hns := hns)
          ρ D.sourceGeneratorLetterBasis D.sourceLocalDim C
          (hletterCard ρ) d (hNFLengthBudget ρ d))
    htotalCardBudget htotalBudget hletterTransfer

/-- Single-letter `NFOfWord` payload together with the explicit transfer bridge
required by the legacy total-normal-form close-out.

This is the final proof-facing shape: the Cook--Levin row is still supplied in
one certified witnessed `NFOfWord` generator-letter basis, while the
compatibility with total `NF` remains a separate explicit span/budget bridge. -/
structure RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSingleLetterGeneratorBasisTransferMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n) extends
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSingleLetterGeneratorBasisMaps
      M n hn2 htb hns where
  sourceTotalNormalFormGeneratorBasis_card_le :
    ∀ (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
          ConstraintType (Nat.log 2 n))
...
/-- Forget the single-letter presentation into the folded-span transfer maps
consumed by the existing final close-out. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps_of_singleLetterGeneratorBasisTransferMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSingleLetterGeneratorBasisTransferMaps
      M n hn2 htb hns) :
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps
      M n hn2 htb hns where
  toRouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisMaps :=
    routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisMaps_of_singleLetterGeneratorBasisMaps
      M n hn2 htb hns
      D.toRouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSingleLetterGeneratorBasisMaps
  sourceTotalNormalFormGeneratorBasis_card_le :=
...
/-- Promote budgeted witnessed `NFOfWord` generator-basis transfer maps to the
standalone transfer data consumed by the final close-out. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordGeneratorBasisTransferData_of_budgetedGeneratorBasisTransferMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisTransferMaps
      M n hn2 htb hns) :
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordGeneratorBasisTransferData
      M n hn2 htb hns where
  profileOfCanonicalWindow := D.profileOfCanonicalWindow
...
/-- Exact seeded-span form of the witnessed `NFOfWord` generator-basis payload.

This is weaker than the seed-or-single-component surface but still paper-faithful:
the row must lie in the span of the finite basis built only from the explicit
empty-word seed and the certified letters of the witnessed `NFOfWord`.  It is
not a common/global span and does not identify `NFOfWord` with total `NF`. -/
structure RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n) extends
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns where
  ...
  sourceSeededProfileGeneratorBudget_le :
    ∀ ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ...
      (Fintype.card
          ...
          sourceLocalDim ρ ≤
        withinProfileBound (Nat.log 2 n)
  leibnizTraceWord_mem_seededNFOfWordGeneratorBasis :
    ∀ (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
          ConstraintType (Nat.log 2 n))
      (S' : List (Fin (n / 3))) (hS : S'.length ≤ Nat.log 2 n)
      (shift : MvPolynomial (Fin (n / 3)) ℚ)
...
/-- Exact seeded-span payload together with explicit transfer into the legacy
total-normal-form close-out. -/
structure RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanTransferMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n) extends
...
/-- End-to-end constructor from witnessed `NFOfWord` local-basis maps to the
seeded-span final transfer surface.

This is the current paper-faithful close-out interface below the final theorem:
local-type row membership is used to extract exact `NFOfWord` generator-basis
row membership, that row is embedded into the seeded witnessed span, and only
explicit budget/transfer obligations remain. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanTransferMaps_of_budgetedLocalBasisMaps_and_seededTransferBudgets
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns)
    ...
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanTransferMaps
      M n hn2 htb hns :=
  let S :=
    routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanMaps_of_generatorBasisMaps_and_emptySeed
      M n hn2 htb hns
      (routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordBudgetedGeneratorBasisMaps_of_budgetedLocalBasisMaps_and_generatorBudgets
        M n hn2 htb hns D hNFCard hNFProfileBudget)
      emptyBasis hseededCard hseededBudget
  { toRouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanMaps := S
    sourceTotalNormalFormGeneratorBasis_card_le := htotalCard
    sourceTotalFiniteEndBudget_le := htotalBudget
    seededNFOfWordGeneratorBasis_span_le_totalNormalFormBasis := by
      intro ρ d
      exact
        routeBPaperFaithfulTPhi_strictSource_seededNFOfWordGeneratorBasis_span_le_totalNormalFormBasis_of_seed_and_letterwise_mem
          (M := M) (n := n) (hn2 := hn2) (htb := htb) (hns := hns)
          ρ emptyBasis D.sourceGeneratorLetterBasis d
          (hseedTransfer ρ d) (hletterTransfer ρ d) }

/-- With empty seed, the seeded witnessed `NFOfWord` basis is definitionally the
ordinary witnessed-letter basis.  This keeps the zero-letter case explicit while
not adding any fake basis element. -/
theorem routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis_empty_seed_eq
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n))
  ...
  unfold routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis
  unfold routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordGeneratorLetterBasis
  unfold routeBPaperFaithfulTPhi_seededAttachFoldedUnion
  unfold routeBPaperFaithfulTPhi_seededFoldedUnion
  rfl

/-- End-to-end constructor with the empty-word seed specialized to `∅`.

This removes the seed-cardinality and seed-transfer obligations entirely: the
empty seed contributes no basis elements and transfers trivially.  The explicit
seeded route is still used, so the empty `Σ*` case is not erased. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanTransferMaps_of_budgetedLocalBasisMaps_emptySeed
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns)
    ...
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanTransferMaps
      M n hn2 htb hns :=
  routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanTransferMaps_of_budgetedLocalBasisMaps_and_seededTransferBudgets
    M n hn2 htb hns D (fun _ => ∅) hNFCard hNFProfileBudget
    (by
      intro ρ d
      simpa [routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis_empty_seed_eq]
        using hNFCard ρ d)
    hseededBudget htotalCard htotalBudget
    (by intro ρ d x hx; simp at hx)
    hletterTransfer

/-- Constructor for the final seeded-span transfer maps from atomic
paper-faithful obligations.

This is the natural finite-basis instantiation surface: prove the row directly
in the span of the exact seeded witnessed `NFOfWord` basis, then supply explicit
cardinality and transfer obligations.  Unlike the seed-or-single-component
constructor below, this allows the row proof to be a genuine finite linear
combination over the seed plus witnessed letters, while still forbidding any
common/global span or total-`NF` shortcut. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanTransferMaps_of_atomicSeededSpanData
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns)
  ...
  toRouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps := D
  sourceEmptyWordBasis := emptyBasis
  sourceSeededNFOfWordGeneratorBasis_card_le := by
    intro ρ d
    exact
      routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis_card_le_of_seed_length_budget
        (M := M) (n := n) (hn2 := hn2) (htb := htb) (hns := hns)
        ρ emptyBasis D.sourceGeneratorLetterBasis D.sourceLocalDim C
        (hletterCard ρ) d (hseededCardBudget ρ d)
  sourceSeededProfileGeneratorBudget_le := hseededProfileBudget
  leibnizTraceWord_mem_seededNFOfWordGeneratorBasis := hrowSpan
  sourceTotalNormalFormGeneratorBasis_card_le := htotalCard
  sourceTotalFiniteEndBudget_le := htotalBudget
  seededNFOfWordGeneratorBasis_span_le_totalNormalFormBasis := by
    intro ρ d
...
/-- Constructor for the final seeded `NFOfWord` transfer maps from atomic
paper-faithful obligations.

This is the intended remaining instantiation surface: provide the explicit
zero-letter basis, per-letter budget, row membership in either seed or witnessed
letter, and separate seed/letter transfer into the downstream total-normal-form
basis.  The constructor performs only cardinality/span bookkeeping. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisTransferMaps_of_atomicSeedAndLetterData
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns)
        ...
        (hlen : ∑ i : Fin ((cookLevinFactorList M n hn2 htb hns).length),
          ...
          row ∈ emptyBasis ρ ∨
            ∃ g : RouteBPaperFaithfulTPhiFiniteEnd
              ...
              ∃ hg : g ∈ routeBPaperFaithfulTPhi_strictSourceLeibnizTraceNFOfWord
                  M n hn2 htb hns d,
                row ∈ D.sourceGeneratorLetterBasis ρ
                  ⟨g,
                    routeBPaperFaithfulTPhi_strictSourceLeibnizTraceNFOfWord_letters_mem_generators
                      M n hn2 htb hns d g hg⟩)
    (htotalCard :
  ...
  sourceSeededNFOfWordGeneratorBasis_card_le := by
    ...
    exact
      routeBPaperFaithfulTPhi_strictSourceTraceNFOfWordSeededGeneratorLetterBasis_card_le_of_seed_length_budget
        ...
        (hletterCard ρ) d (hseededCardBudget ρ d)
  sourceSeededProfileGeneratorBudget_le := hseededProfileBudget
  leibnizTraceWord_mem_seedOrSingleNFOfWordGeneratorComponent := hrow
  sourceTotalNormalFormGeneratorBasis_card_le := htotalCard
  sourceTotalFiniteEndBudget_le := htotalBudget
  seededNFOfWordGeneratorBasis_span_le_totalNormalFormBasis := by
    intro ρ d
...
The older generator-basis data asks for cardinality of the exact `NFOfWord`
fold for every arbitrary derivative distribution `d`.  The final trace-letter
route does not need that universal `d` budget: it needs row membership for the
witnessed row and the total-normal-form basis/cardinality budget.  This surface
therefore keeps only the faithful row extraction from the witnessed local basis
plus explicit total-`NF` compatibility. -/
structure RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordDirectTransferMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n) extends
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns where
...
/-- Constructor for the direct exact-`NFOfWord` final-transfer surface from
budgeted local-basis maps plus total/per-letter transfer obligations.

Row membership is extracted from the witnessed local basis; the universal exact
`NFOfWord` cardinality field is intentionally absent. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizNFOfWordDirectTransferMaps_of_budgetedLocalBasisMaps_and_transferBudgets
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordBudgetedLocalBasisMaps
      M n hn2 htb hns)
...
/-- Forget exact seeded-span transfer maps into trace-letter final data. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizTraceLetterBasisData_of_seededNFOfWordGeneratorBasisSpanTransferMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordSeededGeneratorBasisSpanTransferMaps
      M n hn2 htb hns) :
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizTraceLetterBasisData
      M n hn2 htb hns where
  profileOfCanonicalWindow := D.profileOfCanonicalWindow
...
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizTraceLetterBasisData_of_directNFOfWordTransferMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizNFOfWordDirectTransferMaps
      M n hn2 htb hns) :
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizTraceLetterBasisData
      M n hn2 htb hns where
  profileOfCanonicalWindow := D.profileOfCanonicalWindow
...
noncomputable def RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizLocalMonoidNormalForms.sourceAlphabet
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2}
    {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizLocalMonoidNormalForms
      M n hn2 htb hns)
    (ρ : RouteBPaperFaithfulTPhiStrictInterfaceAnonymousProfiles
      ConstraintType (Nat.log 2 n)) :
    ZeroProfileLocalTypeAlphabet (n / 3) (Nat.log 2 n) where
  type := D.sourceNormalForm ρ
  typeFintype := D.sourceNormalFormFintype ρ
  localDim := D.sourceLocalDim ρ
  localBasis := D.sourceLocalBasis ρ
  localBasis_card_le := D.sourceLocalBasis_card_le ρ
  profileSymmetricPowerBudget_le := by
    letI := D.sourceNormalFormFintype ρ
    exact D.sourceProfileSymmetricPowerBudget_le ρ

/-- Forget the explicit finite local-monoid normal forms into the witnessed
local-type map surface.  This preserves the witness word and only changes the
container expected by the downstream Route B assembly. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceWitnessedLeibnizLocalTypeMaps_of_localMonoidNormalForms
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (D : RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizLocalMonoidNormalForms
      M n hn2 htb hns) :
    RouteBPaperFaithfulTPhiStrictSourceWitnessedLeibnizLocalTypeMaps
      M n hn2 htb hns where
  profileOfCanonicalWindow := D.profileOfCanonicalWindow
  sourceAlphabet := fun ρ => D.sourceAlphabet ρ
  leibnizWitnessType := by
    intro ρ S' hS shift hshift d hd_elts hlen
    letI := D.sourceNormalFormMonoid ρ
    letI := D.sourceNormalFormFintype ρ
    letI := D.sourceNormalFormDecidableEq ρ
    exact
      ((PallLean.Paper93.NF (D.sourceGenerators ρ)
        ((D.leibnizWitnessWord ρ S' hS shift hshift d hd_elts hlen).prod)).prod)
...
This is the same atomic local algebra as the older Leibniz map surface, but
indexed by the selected canonical-window profile.  The construction only uses
source support transport from the ambient strict first-of-block support fact; it
does not collapse Leibniz terms to one derivative-count profile and does not use
any ambient all-profile supremum. -/
noncomputable def routeBPaperFaithfulTPhi_strictSourceLeibnizLocalTypeCompressionData_of_leibnizMaps
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2)
    (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (profileOfCanonicalWindow :
      ∀ w : PallLean.Paper93.Window
  ...
  leibnizTermType := by
    ...
    exact (leibnizMap ρ).rowType S' (le_of_eq hSlen) shift
      (routeBPaperFaithfulTPhi_sourceShift_vars_subset_of_renamed_subset
        n S' shift hshiftVars)
  leibnizTerm_mem_localTypeSpace := by
    intro ρ S' shift α hSlen hshiftDegree hshiftVars hadm hrow hρ g hg
    exact (leibnizMap ρ).leibnizTerm_mem_typeSpace
      S' (le_of_eq hSlen) shift
      (routeBPaperFaithfulTPhi_sourceShift_vars_subset_of_renamed_subset
