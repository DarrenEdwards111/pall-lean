...
theorem rename_branch_transport_target_via_bridgeMapU_consequence_of_semantic
    ...
    ≤ Submodule.map T
        (mlBlockedSpdpSubspace (latentPartition M n) (Nat.log 2 n) (Nat.log 2 n)
          (latentCompiledPoly M n)) :=
  rename_branch_transport_target_via_bridgeMapU_of_source_membership M n h_le B T
    (rename_branch_generator_transport_semantic M n h_le T)
...
/-- Direct concrete source-membership theorem for the earliest bridge-map
wrapper on the preferred `restrictPoly` route. This is the missing raw
bridge-shaped sibling of `rename_branch_transport_target_of_U_source_membership_for_restrictPoly`. -/
theorem rename_branch_transport_target_via_bridgeMapU_of_source_membership_for_restrictPoly
    (M : DTM) (n : ℕ)
    (h_le : npNumVars n ≤ numVars M n (Nat.log 2 n))
    (B : FullToLatentBridge M n) :
    Submodule.map (MvPolynomial.rename (witnessInclusion M n h_le)).toLinearMap
    ...
    ≤ Submodule.map (MultilinearSPDP.restrictPoly ℚ B.toLatent B.inj).toLinearMap
        (mlBlockedSpdpSubspace (latentPartition M n) (Nat.log 2 n) (Nat.log 2 n)
          (latentCompiledPoly M n)) :=
  rename_branch_transport_target_via_bridgeMapU_of_source_membership M n h_le B
    (MultilinearSPDP.restrictPoly ℚ B.toLatent B.inj).toLinearMap
    (fun S m hLen hdeg hvars hadm =>
      by
        refine ⟨(mapFullToLatentPoly M n B).toLinearMap
          ((MvPolynomial.rename (witnessInclusion M n h_le))
            (mlProj (m * SPDP.iterDerivList S (tseitinPoly ℚ n)))), ?_, ?_⟩
        · exact rename_branch_globalDomStyleU_for_bridgeMap M n h_le B S m hLen hdeg hvars hadm
...
theorem mlBlockedSpdpSubspace_fullCompiled_le_map_via_semantic_targets_of_semantic
  ...
  exact mlBlockedSpdpSubspace_violation_le_map_of_semantic M n B T hViolMatches hSem
...
/-- Direct concrete source-membership theorem for the earliest bridge-map
wrapper on `bridgeReconstructionMap`. This collapses the new raw `restrictPoly`
bridge theorem through `bridgeReconstructionMap = restrictPoly`. -/
theorem rename_branch_transport_target_via_bridgeMapU_of_source_membership_for_bridgeReconstructionMap
    (M : DTM) (n : ℕ)
    (h_le : npNumVars n ≤ numVars M n (Nat.log 2 n))
    (B : FullToLatentBridge M n) :
    Submodule.map (MvPolynomial.rename (witnessInclusion M n h_le)).toLinearMap
...
/-- Direct concrete bridge-reconstruction sibling of the earliest bridge-map
source-image theorem. This keeps the raw bridge-shaped source-image route on
`bridgeReconstructionMap` itself, not only via the later consequence name. -/
theorem rename_branch_transport_target_via_bridgeMapU_of_source_image_for_bridgeReconstructionMap
    (M : DTM) (n : ℕ)
    (h_le : npNumVars n ≤ numVars M n (Nat.log 2 n))
    (B : FullToLatentBridge M n) :
    Submodule.map (MvPolynomial.rename (witnessInclusion M n h_le)).toLinearMap
...
theorem rename_branch_transport_target_via_bridgeMapU_of_semantic_membership_consequence_of_source_image_for_bridgeReconstructionMap
    ...
    ≤ Submodule.map (bridgeReconstructionMap M n B)
        (mlBlockedSpdpSubspace (latentPartition M n) (Nat.log 2 n) (Nat.log 2 n)
          (latentCompiledPoly M n)) :=
  rename_branch_transport_target_via_bridgeMapU_of_source_image_for_bridgeReconstructionMap M n h_le B
...
theorem rename_branch_transport_target_via_bridgeMapU_consequence_of_compiledWitness
    ...
    ≤ Submodule.map T
        (mlBlockedSpdpSubspace (latentPartition M n) (Nat.log 2 n) (Nat.log 2 n)
          (latentCompiledPoly M n)) :=
  rename_branch_transport_target_via_bridgeMapU_compiledWitness_consequence_of_semantic M n h_le T
...
theorem mlBlockedSpdpSubspace_fullCompiled_le_map_via_bridgeMapU_of_target_consequence_for_bridgeReconstructionMap_of_assignToLatent
    ...
    ≤ Submodule.map (bridgeReconstructionMap M n B)
        (mlBlockedSpdpSubspace (latentPartition M n) (Nat.log 2 n) (Nat.log 2 n)
          (latentCompiledPoly M n)) :=
  mlBlockedSpdpSubspace_fullCompiled_le_map_via_bridgeMapU_of_target_consequence M n h_le B
    hAssignToLatent hViolMatches
...
theorem mlBlockedSpdpSubspace_fullCompiled_le_map_for_bridgeReconstructionMap_consequence
    ...
    ≤ Submodule.map (bridgeReconstructionMap M n B)
        (mlBlockedSpdpSubspace (latentPartition M n) (Nat.log 2 n) (Nat.log 2 n)
          (latentCompiledPoly M n)) :=
  mlBlockedSpdpSubspace_fullCompiled_le_map_via_bridgeMapU_of_target_consequence_for_bridgeReconstructionMap_of_assignToLatent
    M n h_le B hAssignToLatent hViolMatches
...
/-- Honest one-way consequence replacing the old proof-term equality pin:
the concrete `bridgeReconstructionMap` full-compiled endpoint follows from the
preferred `restrictPoly` route after rewriting the target map, but the two `≤`
proof terms should not be identified by definitional equality. -/
theorem mlBlockedSpdpSubspace_fullCompiled_le_map_for_bridgeReconstructionMap_consequence_of_restrictPoly
    (M : DTM) (n : ℕ)
    (h_le : npNumVars n ≤ numVars M n (Nat.log 2 n))
    (B : FullToLatentBridge M n)
    (hViolMatches :
    ...
    ≤ Submodule.map (bridgeReconstructionMap M n B)
        (mlBlockedSpdpSubspace (latentPartition M n) (Nat.log 2 n) (Nat.log 2 n)
          (latentCompiledPoly M n)) := by
  rw [bridgeReconstructionMap_eq_restrictPoly_target M n B]
  exact mlBlockedSpdpSubspace_fullCompiled_le_map_for_restrictPoly M n h_le B hViolMatches
