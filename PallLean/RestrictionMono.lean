...
/-- The SPDP subspace of the restricted polynomial is contained in
    the image of the SPDP subspace of f under applyRestriction ρ.

    Each generator m * iterDerivList S (applyRestriction ρ f) of the
    target subspace is either 0 (fixed var in S) or
    m * applyRestriction ρ (iterDerivList S f) (free S).

    In the free case, this equals applyRestriction ρ (m' * iterDerivList S f)
    where m' is any polynomial with applyRestriction ρ m' = m. Since m itself
    satisfies this (free variables map to themselves), we can take m' = m.

    Wait — applyRestriction ρ m ≠ m in general (m may use fixed vars).
    But m * applyRestriction ρ d ∈ span{applyRestriction ρ m'' * applyRestriction ρ d}
    because {applyRestriction ρ m'' : deg m'' ≤ ℓ} generates all polys of degree ≤ ℓ
    (restriction is surjective on the polynomial ring).

    Actually simpler: the target subspace is generated by
    {m * applyRestriction ρ (iterDerivList S f) : free S, deg m ≤ ℓ}.
    For each generator, m * applyRestriction ρ d_S is in the image of the
    linear map (q ↦ m * applyRestriction ρ q) applied to iterDerivList S f.
    But this isn't a SINGLE linear map for all generators.

    Correct approach: show target generators lie in image of restriction. -/
private theorem spdp_restricted_le_image {n : ℕ}
    (ρ : VarRestriction n) (f : MvPolynomial (Fin n) ℚ) (κ ℓ : ℕ) :
    spdpSubspace κ ℓ (applyRestriction ρ f) ≤
      Submodule.map ((applyRestrictionAlgHom ρ).toLinearMap) (spdpSubspace κ ℓ f) := by
  apply Submodule.span_le.mpr
  intro q ⟨S, m, hlen, hdeg, hq⟩
  rw [hq, iterDerivList_applyRestriction ρ S f]
  split
  · -- Free S: m * applyRestriction ρ (iterDerivList S f)
    rename_i hfree
    -- = applyRestriction ρ (applyRestriction ρ⁻¹(m) * iterDerivList S f)
    -- But we don't have an inverse. Instead, note:
    -- m * applyRestriction ρ d ∈ image of applyRestriction ρ
    -- because m * applyRestriction ρ d = applyRestriction ρ (m * d)
    -- when m uses only free variables. But m might use fixed variables!
    -- In that case, applyRestriction ρ m ≠ m.
    -- However, m * (applyRestriction ρ d) IS still a polynomial.
    -- We need it in the IMAGE of restriction from spdpSubspace.
    -- Key: m * applyRestriction ρ d = applyRestriction ρ (m * d)
    -- iff applyRestriction ρ m = m. This fails for fixed-var m.
    -- So we use: m * applyRestriction ρ d ∈ applyRestriction ρ '' (whole ring)
    -- and the whole ring image contains everything (restriction is surjective).
    -- But we need it in the IMAGE of spdpSubspace specifically.
    sorry
  · -- Fixed var in S: result is m * 0 = 0
    rw [mul_zero]
...
theorem spdpRank_restriction_mono {n : ℕ}
  ...
  calc Module.finrank ℚ (spdpSubspace κ ℓ (applyRestriction ρ f))
      ≤ Module.finrank ℚ (Submodule.map ((applyRestrictionAlgHom ρ).toLinearMap) (spdpSubspace κ ℓ f)) :=
        Submodule.finrank_mono (spdp_restricted_le_image ρ f κ ℓ)
    _ ≤ Module.finrank ℚ (spdpSubspace κ ℓ f) :=
