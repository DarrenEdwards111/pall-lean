...
/-! ## Same-Target NP Lower-Bound Interface

The paper-faithful NP side is a lower bound on the exact coupled sheet selected
by the semantic extraction theorem. The interface below deliberately has no
polynomial projection map: it records identity-minor evidence directly for the
target's own block partition and polynomial. -/

/-- Strong NP-side lower bound on the exact Route B extraction target. -/
def GodMoveSameTargetStrongNPLower
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2} {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (target : GodMoveExtractionTarget M n hn2 htb hns) : Prop :=
  Nat.choose (n / 3) (Nat.log 2 n) ≤
...
/-- Same-target identity-minor evidence for a semantic extraction target.

This is the narrow Theorem-207 NP-side seam: callers must prove that the
identity minor generated by the hard instance has enough rows, and that those
rows are linearly independent in the SPDP space of this exact target. No
arbitrary projection or transport wrapper is part of the statement. -/
structure RouteBIdentityMinorSameTargetData
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2} {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (target : GodMoveExtractionTarget M n hn2 htb hns) : Type where
  minorSize : ℕ
  target_choose_le_minorSize :
    Nat.choose (n / 3) (Nat.log 2 n) ≤ minorSize
  identity_minor_rank_lower :
    minorSize ≤
      mlBlockedSpdpRank target.coupledPartition
        (Nat.log 2 n) (Nat.log 2 n) target.coupledPoly
...
identity-minor evidence. -/
def toRouteB_Obligations_from_same_target_identity_minor
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2} {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    {hdec : DecidesSAT M}
    (targetData : GodMoveExtractionTargetData M n hn2 htb hns hdec)
    (minor : RouteBIdentityMinorSameTargetData targetData.extractionTarget) :
    GodMoveRouteB_Obligations M n hn2 htb hns :=
  targetData.toRouteB_Obligations
    (routeB_strong_np_from_same_target_identity_minor minor)

end GodMoveExtractionTargetData

/-- Package Route B obligations from a semantic gap plus same-target
identity-minor evidence on the gap's extracted target. -/
def routeB_from_semantic_gap_same_target_identity_minor
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2} {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    {hdec : DecidesSAT M}
    (gap : GodMoveSemanticGap M n hn2 htb hns hdec)
    (minor : RouteBIdentityMinorSameTargetData gap.extractionTarget) :
    GodMoveRouteB_Obligations M n hn2 htb hns :=
  routeB_from_semantic_gap gap
    (routeB_strong_np_from_same_target_identity_minor minor)

@[simp] theorem routeB_from_semantic_gap_same_target_identity_minor_extractionTarget
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2} {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    {hdec : DecidesSAT M}
    (gap : GodMoveSemanticGap M n hn2 htb hns hdec)
    (minor : RouteBIdentityMinorSameTargetData gap.extractionTarget) :
    (routeB_from_semantic_gap_same_target_identity_minor gap minor).extractionTarget =
      gap.extractionTarget := rfl
...
/-- PD-matrix NP data tied to the exact semantic extraction target.

This is the same-target version of `RouteBNPFromPdMatrix`: it allows the PD
lower-bound route only when the PD polynomial and block partition are
definitionally identified with the chosen Route B target, rather than routed
through an unconstrained projection wrapper. -/
structure RouteBNPFromPdMatrixSameTarget
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2} {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    (target : GodMoveExtractionTarget M n hn2 htb hns)
    extends RouteBNPFromPdMatrix n target.coupledVars where
  blockPart_eq_target : blockPart = target.coupledPartition
  poly_eq_target : poly = target.coupledPoly

/-- Same-target PD data gives the weakened NP lower bound on the exact target. -/
theorem routeB_weakened_np_from_same_target_pdMatrix
    {M : DTM} {n : ℕ} {hn2 : n ≥ 2} {htb : M.timeBound ≤ 4} {hns : M.numStates ≤ n}
    {target : GodMoveExtractionTarget M n hn2 htb hns}
    (d : RouteBNPFromPdMatrixSameTarget target) :
    n ^ (Nat.log 2 n / 4) ≤
      mlBlockedSpdpRank target.coupledPartition
        (Nat.log 2 n) (Nat.log 2 n) target.coupledPoly := by
  have h := routeB_weakened_np_from_pdMatrix d.toRouteBNPFromPdMatrix
  simpa [d.blockPart_eq_target, d.poly_eq_target] using h
...
/-- Same-target identity-minor variant of `separation_from_semantic_target`.

This is the final contradiction shell when the strong NP lower bound is
provided through `RouteBIdentityMinorSameTargetData` on the semantic target
itself. -/
theorem separation_from_semantic_target_same_target_identity_minor
    (M : DTM) (n : ℕ) (hn2 : n ≥ 2) (htb : M.timeBound ≤ 4) (hns : M.numStates ≤ n)
    (hdec : DecidesSAT M) (hn804 : n ≥ 2 ^ 804)
    (target : GodMoveExtractionTarget M n hn2 htb hns)
    (hsem : GodMoveExtractionSemanticObligation M n hn2 htb hns hdec target)
    (minor : RouteBIdentityMinorSameTargetData target)
    (bridge :
      ∀ sem : ExtractionMapSemantics M n hn2 htb hns hdec target,
        ExtractionMapRankBridge sem)
    (hP : mlBlockedSpdpRank
      ...
      (compiledPoly (cook_levin_compilation M n hn2 htb hns)) ≤ n ^ 200) :
    False :=
  separation_from_semantic_target_strong_np M n hn2 htb hns hdec hn804 target hsem
    (routeB_strong_np_from_same_target_identity_minor minor) bridge hP
...
-- Expected: propext, Classical.choice, Quot.sound (NO custom axioms)
-- Same-target specialization of the PD-matrix route.
...
#print axioms separation_from_semantic_target_same_target_identity_minor
-- Expected: propext, Classical.choice, Quot.sound (NO custom axioms)
-- Same exact-target shell, with strong NP supplied as identity-minor data.
