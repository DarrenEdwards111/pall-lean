...
private theorem iterDerivList_mul_const_left_copy
...
/-- Full copy-slot iterated derivative formula on a finite copyCon product. -/
theorem iterDeriv_copyConProd_eq (M : DTM) (n : ℕ)
    (ks : List (Fin (latentBaseVars M n))) (hnd : ks.Nodup)
    (s : Finset (Fin (latentBaseVars M n))) (hks : ∀ k ∈ ks, k ∈ s) :
    iterDerivList (ks.map (copySlot M n)) (∏ i ∈ s, copyConGadget M n i) =
  ...
  | cons k rest ih =>
      ...
      rw [show iterDerivList (copySlot M n k :: rest.map (copySlot M n)) (∏ i ∈ s, copyConGadget M n i) =
          iterDerivList (rest.map (copySlot M n))
            (pderiv (copySlot M n k) (∏ i ∈ s, copyConGadget M n i))
        from by unfold iterDerivList; simp [List.foldl]]
      have hk_mem : k ∈ s := hks k (by simp)
      rw [pderiv_copySlot_copyConProd_of_mem M n s k hk_mem]
      have hconst : ∀ i ∈ rest.map (copySlot M n), pderiv i (-(Xcon M n k)) = 0 := by
        intro i hi
...
theorem iterDeriv_copyConSheet_eq (M : DTM) (n : ℕ)
    (ks : List (Fin (latentBaseVars M n))) (hnd : ks.Nodup) :
    iterDerivList (ks.map (copySlot M n)) (copyConSheet M n) =
      C ((-1 : ℚ)^ks.length) * (ks.map (Xcon M n)).prod *
        (∏ i ∈ (Finset.univ \ ks.toFinset), copyConGadget M n i) := by
  unfold copyConSheet
  exact iterDeriv_copyConProd_eq M n ks hnd Finset.univ (by intro k hk; simp)
