...
/-- Canonical restriction witness (paper §32.3), intended to model the
explicit derandomized choice `ρ*` used in Steps 4–5.

Kept abstract here so we do not accidentally hard-code the all-free scaffold,
which is inconsistent with the intended depth-collapse behavior. -/
axiom canonicalRestriction (M : DTM) (n : ℕ) :
