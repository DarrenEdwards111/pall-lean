...
Derived from generator correspondence target. -/
axiom compiled_subspace_element_transport_target
    (M : DTM) (n : ℕ)
    (h_le : npNumVars n ≤ numVars M n (Nat.log 2 n))
    (T : MvPolynomial (Fin (latentNumVars M n)) ℚ →ₗ[ℚ]
      MvPolynomial (Fin (numVars M n (Nat.log 2 n))) ℚ) :
...
is contained in the transported latent SPDP subspace at logscale, derived from
the elementwise transport target. -/
theorem compiled_subspace_le_latent_subspace_logscale_target
    (M : DTM) (n : ℕ)
    (h_le : npNumVars n ≤ numVars M n (Nat.log 2 n))
    (T : MvPolynomial (Fin (latentNumVars M n)) ℚ →ₗ[ℚ]
      MvPolynomial (Fin (numVars M n (Nat.log 2 n))) ℚ) :
  ...
  intro q hq
  rcases compiled_subspace_element_transport_target M n h_le T
    (compiled_generator_transport_target M n h_le T)
    q hq with ⟨r, hr, hqr⟩
  refine ⟨r, hr, ?_⟩
  simpa [hqr]
