...
| Smolensky **dimension** bound (#low-degree monomials) | ✓ count built (`ComputationalDepthLayer3DimensionCount.lean`): `lowDegMonomials_card` = `∑_{k≤D} C(n,k)`; `_full` = `2^n`, `_le_two_pow`, `_lt_two_pow` (strict for `D<n`); `_halfway` exact `2^{2m}=2^{n-1}` at the half-degree; **`_halfway_margin`** — `4·count < 3·2^n` at `D=⌊n/2⌋` (Δ=0), i.e. count `< (3/4)·2^n` *with no central-binomial estimate* (the base case); `boolToZMod_pow_succ` (`x^{e+1}=x`) the multilinear lever; **`centralBinom_sq_le`** — the **central-binomial `√n` bound** `(2m+1)·C(2m,m)² ≤ 4^(2m)` (= `C(2m,m) ≤ 4^m/√(2m+1)`), the Stirling-type estimate Mathlib lacked, proved by induction via `Nat.succ_mul_centralBinom_succ` (step `(2m+3)(2m+1) ≤ 4(m+1)²`). **Remaining:** assemble the `Δ≥1` band margin — `∑_{k≤m+Δ} C(2m+1,k) = 2^{2m} + (band ≤ Δ·C(2m+1,m))`, with `(m+1)·C(2m+1,m) = (2m+1)·C(2m,m)` + `centralBinom_sq_le` bounding `C(2m+1,m)`, gives `< (3/4)·2^n` for `Δ = O(√m)` |
