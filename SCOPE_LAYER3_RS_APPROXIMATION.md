...
| **recursive agreement lift** (OR/AND/NOT/leaf) | ✓ built (`ComputationalDepthLayer3Agreement.lean`): `toAgree` faithful single-function approximant (`termination_by sizeOf`, clean `Fin cs.length` indexing; `∨`→`genOrApprox`, `∧`→De Morgan `1 - genOrApprox(1 - child)`, `¬`→`1-child`, leaves→`X`/`C`); `AgreeGood` per-input goodness (per `∨`/`∧` gate: when gate fires, some sampled form over the relevant true values is nonzero; `MOD` excluded as `False`); `orGate_eval_iff`/`andGate_eval_iff` (`.any`/`.all` ↔ indexed ∃/∀); `toAgree_eval` — **AgreeGood x R C ⇒ eval_x(toAgree C) = boolToZMod(C.eval x)** by structural induction. The circuit approximant computes the circuit exactly on every *good* input. **Remaining:** MOD gate (exact Fermat indicator + modular count) + probabilistic "most inputs good" + `MOD_q`-reduction + band margin |
