...
| **marginalisation** (per-gate column sums factor) | ✓ built (`ComputationalDepthLayer3Averaging.lean`): `sum_proj_eq` — over a product form space `Φ = ∀ i, A i`, a single-coordinate sum factors `∑_f g(f i₀) = (∏_{i≠i₀}\|A i\|)·∑_a g a` (via `Equiv.piSplitAt` + `Fintype.sum_prod_type`). The factorisation that turns a per-gate single-coordinate bound into the joint `Bᵢ` of `exists_form_total_errors`. **Remaining (architectural):** the gate-position-indexed approximant over `Φ` (so each `gbad i` depends on coord `i`) — the structural rung that wires marginalisation + `exists_form_total_errors` to the concrete circuit; + `MOD_q`-reduction + band margin |
