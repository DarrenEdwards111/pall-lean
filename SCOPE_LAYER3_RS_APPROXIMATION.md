...
| **gate eval over child values** (`genOrApprox`) | ✓ built (same file): `genOrApprox_eval` — `eval pt (genOrApprox p R q) = 0` iff every form `∑_j R s j · eval pt (q_j)` vanishes, else `1` (Fermat); `genOrApprox_eval_eq_orApprox_eval` recovers the variable case. The gate-level agreement step a recursive circuit lift consumes (children's *evaluated values* as inputs). **Remaining:** the full structural induction threading per-gate goodness up the circuit + probabilistic choice of forms + `MOD_q`-reduction + band margin |
