...
| **algebraic lever** (monomials closed under × on cube) | ✓ built (same file): `squarefreeEvalMonomial_mul` — `e_S·e_T = e_{S∪T}` (overlap absorbed by `x²=x`); `squarefreeEvalMonomial_empty` — `e_∅ = 1`; `squarefreeEvalMonomial_mul_card_le` — degree **subadditive** `deg(e_S·e_T) = |S∪T| ≤ |S|+|T|`. Function-level form of the `boolToZMod_sq`/`_pow_succ` lever; the algebraic engine for the deferred composition step |
