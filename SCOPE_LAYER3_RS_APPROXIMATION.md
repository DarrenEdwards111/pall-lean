...
| **circuit-level averaging** (joint form space) | ✓ built (same file): `exists_le_sum_of_sum_le` (first moment over a sum of per-gate error functions); `exists_form_total_errors` — with `Φ` the joint form space, composed bad ⊆ ∪ per-gate bads (`hsub`) and per-gate column-sum bounds `Bᵢ`, **∃ a single joint form `ω` with `Φ.card·\|cbad ω\| ≤ ∑ᵢ Bᵢ`** (composed-error rate `≤ (∑Bᵢ)/\|Φ\|`). Combines per-gate averaging + union bound into one form choice — the structural lift. **Remaining:** instantiate to the concrete circuit (per-gate-indexed forms) + `MOD_q`-reduction + band margin |
