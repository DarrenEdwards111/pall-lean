...
| **goodness decomposition** (per-gate local conditions) | ✓ built (`ComputationalDepthLayer3Agreement.lean`): `subcircuits`/`subcircuitsList` enumerate all subcircuits; `localGood` the gate-local condition (`∨`/`∧` form cond, `MOD` `q=p`, else `True`); `mem_subcircuitsList` (child subcircuit ⊆ list); `agreeGood_of_forall` — **(∀ G ∈ subcircuits C, localGood G) ⇒ AgreeGood C** by structural induction; `cbad_subset_gates` — **composed-error set ⊆ `⋃_{G ∈ subcircuits C} {x : ¬localGood G x}`** (the `hsub` of `exists_form_total_errors`, with `gbad G = {¬localGood G}`, gates `= (subcircuits C).toFinset`), via the contrapositive of `agreeGood_of_forall`. *Insight: linearity of expectation needs no per-gate independence — each `localGood` depends only on its fan-in's form coordinate (`sum_proj_eq`).* **Remaining:** finite over-allocated form space `Φ` + `hB` (per-gate column-sum bound via `sum_proj_eq` + a `localGood`-failure count) ⇒ apply `exists_form_total_errors`; + `MOD_q`-reduction + band margin |
