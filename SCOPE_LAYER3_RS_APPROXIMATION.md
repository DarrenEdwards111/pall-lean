...
| **goodness decomposition** (per-gate local conditions) | ✓ built (`ComputationalDepthLayer3Agreement.lean`): `subcircuits`/`subcircuitsList` enumerate all subcircuits; `localGood` the gate-local condition (`∨`/`∧` form cond, `MOD` `q=p`, else `True`); `mem_subcircuitsList` (child subcircuit ⊆ list); `agreeGood_of_forall` — **(∀ G ∈ subcircuits C, localGood G) ⇒ AgreeGood C** by structural induction. Turns `cbad ⊆ {¬AgreeGood}` into `cbad ⊆ ⋃_{gate} {¬localGood}` (union-over-gates for the averaging). *Insight: linearity of expectation needs no per-gate independence — each `localGood` depends only on its fan-in's form coordinate (`sum_proj_eq`).* **Remaining:** assemble the concrete `exists_form_total_errors` instantiation (`gbad`/`cbad`/`Φ` + `hB` via `sum_proj_eq`+`exists_form_few_errors`) + `MOD_q`-reduction + band margin |
